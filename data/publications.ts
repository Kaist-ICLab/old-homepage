interface Publication {
    title: string;
    doi?: string;
    year: number;
    published?: boolean;
    authors: string[];
    venue: string;
    venue_alias: string;
    venue_type?: "Patent" | "Journal" | "Conference" | "Workshop" | "Book";
    paper?: string;
    code?: string;
    slide?: string;
    video?: string;
}

const publications: { [key: string]: Publication } = {
    lee2024sadl: {
        year: 2024,
        title: "S-ADL: Exploring Smartphone-based Activities of Daily Living to Detect Blood Alcohol Concentration in a Controlled Environment",
        doi: "https://doi.org/10.1145/3613904.3642832",
        authors: ["Hansoo Lee", "Auk Kim", "Sang Won Bae", "Uichin Lee"],
        venue_alias: "CHI",
        venue: "Proceedings of the 2024 CHI Conference on Human Factors in Computing Systems",
        published: false,
        paper: "lee2024sadl.pdf"
    },
    lee2024priviaware: {
        year: 2024,
        title: "PriviAware: Exploring Data Visualization and Dynamic Privacy Control Support for Data Collection in Mobile Sensing Research",
        doi: "https://doi.org/10.1145/3613904.3642815",
        authors: ["Hyunsoo Lee", "Yugyeong Jung", "Hei Yiu Law", "Seolyeong Bae", "Uichin Lee"],
        venue_alias: "CHI",
        venue: "Proceedings of the 2024 CHI Conference on Human Factors in Computing Systems",
        published: false,
        paper: "lee2024priviaware.pdf"
    },
    jung2024deepstress: {
        year: 2024,
        title: "DeepStress: Supporting Stressful Context Sensemaking in Personal Informatics Systems Using a Quasi-experimental Approach",
        doi: "https://doi.org/10.1145/3613904.3642766",
        authors: ["Gyuwon Jung", "Sangjun Park", "Uichin Lee"],
        venue_alias: "CHI",
        venue: "Proceedings of the 2024 CHI Conference on Human Factors in Computing Systems",
        published: false,
        paper: "jung2024deepstress.pdf"
    },
    kim2024interrupting: {
        year: 2024,
        title: "Interrupting for Microlearning: Understanding Perceptions and Interruptibility of Proactive Conversational Microlearning Services",
        doi: "https://doi.org/10.1145/3613904.3642778",
        published: false,
        authors: ["Minyeong Kim", "Jiwook Lee", "Youngji Koh", "Chanhee Lee", "Uichin Lee", "Auk Kim"],
        venue_alias: "CHI",
        venue: "Proceedings of the 2024 CHI Conference on Human Factors in Computing Systems",
        paper: "kim2024interrupting.pdf"
    },
    lim2024exploring: {
        year: 2024,
        title: "Exploring Context-Aware Mental Health Self-Tracking Using Multimodal Smart Speakers in Home Environments",
        doi: "https://doi.org/10.1145/3613904.3642846",
        authors: ["Jieun Lim", "Youngji Koh", "Auk Kim", "Uichin Lee"],
        published: false,
        paper: "lim2024exploring.pdf",
        venue_alias: "CHI",
        venue: "Proceedings of the 2024 CHI Conference on Human Factors in Computing Systems",
    },
    lee2024familyscope: {
        year: 2024,
        title: "FamilyScope: Visualizing Affective Aspects of Family Social Interactions using Passive Sensor Data",
        doi: "https://doi.org/10.1145/3637334",
        authors: ["Hyunsoo Lee", "Yugyeong Jung", "Youwon Shin", "Hyesoo Park", "Woohyeok Choi", "Uichin Lee"],
        venue_alias: "CSCW",
        venue: "Proceedings of ACM Conference On Computer-Supported Cooperative Work And Social Computing",
        published: false,
        paper: "lee2024familyscope.pdf"
    },
    jung2024tutorial: {
        year: 2024,
        doi: "https://doi.org/10.1145/3648356",
        title: "A Tutorial on Matching-based Causal Analysis of Human Behaviors Using Smartphone Sensor Data",
        authors: ["Gyuwon Jung", "Sangjun Park", "Eun-Yeol Ma", "Heeyoung Kim", "Uichin Lee"],
        published: true,
        venue_alias: "CSUR",
        venue: "ACM Computing Surveys",
        paper: "jung2024tutorial.pdf"
    },
    koh2023datavisualization: {
        year: 2023,
        doi: "https://doi.org/10.1109/VAHC60858.2023.00017",
        title: "Data Visualization for Mental Health Monitoring in Smart Home Environment: A Case Study",
        authors: ["Youngji Koh", "Chanhee Lee", "Yunhee Ku", "Uichin Lee"],
        venue_alias: "VAHC",
        venue: "2023 Workshop on Visual Analytics in Healthcare (VAHC)",
        published: true,
        paper: "koh2023datavisualization.pdf"
    },
    han2023watchppg: {
        year: 2023,
        title: 'WatchPPG: An Open-Source Toolkit for PPG-based Stress Detection using Off-the-shelf Smartwatches',
        authors: ['Yunjo Han', 'Kobiljon E Toshnazarov', 'Byung Hyung Kim', 'Youngtae Noh', 'Uichin Lee'],
        venue_alias: 'UbiComp',
        venue:
            'Adjunct Proceedings of the 2023 ACM International Joint Conference on Pervasive and Ubiquitous Computing & the 2023 ACM International Symposium on Wearable Computing',
        published: true,
        paper: 'han2023watchppg.pdf',
    },
    park2023measuring: {
        year: 2023,
        title: 'Measuring Device-Specific Physical Activity Trackability in Multi-Device Environments',
        authors: ['Sangjun Park', 'Eunji Park', 'Paul H Lee', 'Uichin Lee'],
        venue_alias: 'BigComp',
        venue:
            '2023 IEEE International Conference on Big Data and Smart Computing (BigComp)',
        published: true,
        paper: 'park2023measuring.pdf',
    },
    lee2023analysis: {
        year: 2023,
        title: 'Analysis system and method for causal inference of digital therapeutics based on mobile data',
        authors: ['Uichin Lee', 'JUNG Gyuwon', 'MA Eun-Yeol', 'Heeyoung Kim'],
        venue_alias: 'Google Patents',
        venue:
            'US Patent',
        published: true,
        paper: 'lee2023analysis.pdf',
    },
    ma2023investigating: {
        year: 2023,
        title: 'Investigating Causality in Mobile Health Data through Deep Learning Models',
        authors: ['Eun-Yeol Ma', 'Heeyoung Kim', 'Uichin Lee'],
        venue_alias: 'BigComp',
        venue:
            '2023 IEEE International Conference on Big Data and Smart Computing (BigComp)',
        published: true,
        paper: 'ma2023investigating.pdf',
    },
    orzikulova2023finderme: {
        year: 2023,
        title: 'FinerMe: Examining App-level and Feature-level Interventions to Regulate Mobile Social Media Use',
        authors: ['Adiba Orzikulova', 'Hyunsung Cho', 'Hye-Young Chung', 'Hwajung Hong', 'Uichin Lee', 'Sung-Ju Lee'],
        venue_alias: 'CSCW',
        venue:
            'Proceedings of ACM Conference On Computer-Supported Cooperative Work And Social Computing',
        published: false,
        paper: 'orzikulova2023finderme.pdf',
    },
    heo2023form: {
        year: 2023,
        title: 'Form to Flow: Exploring Challenges and Roles of Conversational UX Designers in Real-world, Multi-channel Service Environments',
        authors: ['Jeongyun Heo', 'Uichin Lee'],
        venue_alias: 'CSCW',
        venue:
            'Proceedings of ACM Conference On Computer-Supported Cooperative Work And Social Computing',
        published: true,
        paper: 'heo2023form.pdf',
    },
    park2023understanding: {
        year: 2023,
        title: 'Understanding Disengagement in Just-in-Time Mobile Health Interventions',
        authors: ['Joonyoung Park', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2023 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 7, Issue 2, June 2023',
        published: true,
        paper: 'park2023understanding.pdf',
    },
    kang2023k: {
        year: 2023,
        title: 'K-EmoPhone: A Mobile and Wearable Dataset with In-Situ Emotion, Stress, and Attention Labels',
        authors: ['Soowon Kang', 'Woohyeok Choi', 'CheulYoung Park', 'Narae Cha', 'Auk Kim', 'Ahsan Habib Khandoker', 'Leontios Hadjileontiadis', 'Heepyung Kim', 'Yong Jeong', 'Uichin Lee'],
        venue_alias: 'Scientific Data',
        venue: 'Scientific Data Volume 10, No. 1, 2023',
        published: true,
        paper: 'kang2023k.pdf',
    },
    jung2023causal: {
        year: 2023,
        title: 'Causal Analytic Process for Mobile Health Data',
        authors: ['Gyuwon Jung', 'Sangjun Park', 'Uichin Lee', 'Eun-Yeol Ma', 'Heeyoung Kim'],
        venue_alias: 'BigComp',
        venue: '2023 IEEE International Conference on Big Data and Smart Computing (BigComp)',
        published: true,
        paper: 'jung2023causal.pdf',
    },
    lee2023data1: {
        year: 2023,
        title: 'Data-driven Digital Therapeutics Analytics',
        authors: ['Uichin Lee', 'Gyuwon Jung', 'Sangjun Park', 'Eun-Yeol Ma', 'Heeyoung Kim', 'Yonggeon Lee', 'Youngtae Noh'],
        venue_alias: 'BigComp',
        venue: '2023 IEEE International Conference on Big Data and Smart Computing (BigComp)',
        published: true,
        paper: 'lee2023data1.pdf',
    },
    lee2023data2: {
        year: 2023,
        title: 'Data Processing Pipeline of Short-Term Depression Detection with Large-Scale Dataset',
        authors: ['Yonggeon Lee', 'Youngtae Noh', 'Uichin Lee'],
        venue_alias: 'BigComp',
        venue: '2023 IEEE International Conference on Big Data and Smart Computing (BigComp)',
        published: true,
        paper: 'lee2023data2.pdf',
    },
    park2023charlie: {
        year: 2023,
        title:
            'Charlie and the Semi-Automated Factory: Data-Driven Operator Behavior and Performance Modeling for Human-Machine Collaborative Systems',
        authors: ['Eunji Park', 'Yugyeong Jung', 'Inyeop Kim', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue: 'CHI Conference on Human Factors in Computing Systems',
        published: true,
        video: 'https://youtu.be/sNRkhhgoEHI',
        paper: 'park2023charlie.pdf',
    },
    park2023quickref: {
        year: 2023,
        title:
            'QuickRef: Should I Read Cited Papers for Understanding This Paper?',
        authors: ['Sangjun Park', 'Chanhee Lee', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue: 'CHI Conference on Human Factors in Computing Systems',
        published: true,
        slide: 'park2023quickref.pdf',
        video: 'https://youtu.be/SH8Ac7DGWRU',
        paper: 'park2023quickref.pdf',
    },
    jung2023enjoy: {
        year: 2023,
        title:
            'Enjoy, but Moderately!”: Designing a Social Companion Robot for Social Engagement and Behavior Moderation in Solitary Drinking Context',
        authors: ['Yugyeong Jung', 'Gyuwon Jung', 'Sooyeon Jeong', 'Chaewon Kim', 'Woontack Woo', 'Hwajung Hong', 'Uichin Lee'],
        venue_alias: 'CSCW',
        venue: 'Proceedings of ACM Conference On Computer-Supported Cooperative Work And Social Computing',
        published: true,
        paper: 'jung2023enjoy.pdf',
    },
    han2022stressbal: {
        year: 2022,
        title:
            'StressBal: Personalized Just-in-time Stress Intervention with Wearable and Phone Sensing',
        authors: ['Yunjo Han', 'Hyemin Lee', 'Kobiljon Toshnazarov', 'Youngtae Noh', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue: 'Proceedings of the 2022 ACM International Joint Conference on Pervasive and Ubiquitous Computing',
        published: true,
        video: 'https://docs.google.com/presentation/d/1vNkznJx4q5KoOaxMKlzQI5P4IN_leSYscdkZ2WX-KN8/edit?usp=sharing',
        paper: 'han2022stressbal.pdf',
        slide: 'han2022stressbal.pdf'
    },
    choi2023loss: {
        year: 2023,
        title:
            "Loss-Framed Adaptive Microcontingency Management for Preventing Prolonged Sedentariness: Development and Feasibility Study",
        authors: ["Woohyeok Choi", "Uichin Lee"],
        venue_alias: "JMU",
        venue: "JMIR mHealth and uHealth",
        published: true,
        paper: 'choi2023loss.pdf',
    },
    lee2023toward: {
        year: 2023,
        title:
            'Toward Data-Driven Digital Therapeutics Analytics: Literature Review and Research Directions',
        authors: [
            'Uichin Lee',
            'Gyuwon Jung',
            'Eunyeol Ma',
            'Jinsan Kim',
            'Heepyung Kim',
            'Jumabek Alikhanov',
            'Youngtae Noh',
            'Heeyoung Kim',
        ],
        venue_alias: 'IEEE',
        venue: 'IEEE/CAA Journal of Automatica Sinica',
        published: true,
        paper: 'lee2023toward.pdf',
    },
    shin2022smarthome: {
        year: 2022,
        title: "스마트홈 IoT 기술 및 프라이버시 최신 연구 동향",
        authors: ["신유원", "이현수", "최우혁", "김희평", "정용", "이의진"],
        venue_alias: "KCC",
        venue: "2022년 한국컴퓨터종합학술대회(KCC) 논문집",
        published: true,
        paper: "shin2022smarthome.pdf",
        slide: "shin2022smarthome.pdf"
    },
    zitouni2022lstm: {
        year: 2022,
        title:
            'LSTM-Modeling of Emotion Recognition Using Peripheral Physiological Signals in Naturalistic Conversations',
        authors: [
            "M. Sami Zitouni",
            "Cheul Young Park",
            "Uichin Lee",
            "Leontios J. Hadjileontiadis",
            "Ahsan Khandoker"
        ],
        venue_alias: 'IEEE',
        venue: 'IEEE Journal of Biomedical and Health Informatics',
        published: true,
        paper: 'zitouni2022lstm.pdf',
    },
    lee2022trends: {
        year: 2022,
        title:
            'Trends in smart helmets with multimodal sensing for health and safety: scoping review',
        authors: [
            "Peter Lee",
            "Heepyung Kim",
            "M. Sami Zitouni",
            "Ahsan Khandoker",
            "Herbert F. Jelinek",
            "Leontios J. Hadjileontiadis",
            "Uichin Lee",
            "Yong Jeong"
        ],
        venue_alias: 'JMU',
        venue: 'JMIR mHealth and uHealth',
        published: true,
        paper: 'lee2022trends.pdf',
    },
    choi2022you: {
        year: 2022,
        title:
            'You Are Not Alone: How Trending Stress Topics Brought \#Awareness and \#Resonance on Campus',
        authors: [
            "Ryuhaerang Choi",
            "Chanwoo Yun",
            "Hyunsung Cho",
            "Hwajung Hong",
            "Uichin Lee",
            "Sung-ju Lee"
        ],
        venue_alias: 'CSCW',
        venue: 'Proceedings of the ACM on Human-Computer Interaction',
        published: true,
        paper: 'choi2022you.pdf',
        video: 'https://youtu.be/u_lSLpPVbFg',
    },
    lee2022towarddynamic: {
        year: 2022,
        title:
            'Toward Dynamic Consent for Privacy-Aware Pervasive Health and Well-being: A scoping review and research directions',
        authors: ['Hyunsoo Lee', 'Uichin Lee'],
        venue_alias: 'IEEE PerCom',
        venue: 'IEEE International Conference on Pervasive Computing and Communications, 2022',
        published: true,
        paper: 'lee2022towarddynamic.pdf',
    },
    lee2022lv: {
        year: 2022,
        title:
            'LV-Linker: Supporting Fine-grained User Interaction Analyses by Linking Smartphone Log and Recorded Video Data',
        authors: [
            "Hansoo Lee",
            "Sangwook Lee",
            "Youngji Koh",
            "Uichin Lee"
        ],
        venue_alias: 'UIST',
        venue: 'Adjunct Proceedings of the 35th Annual ACM Symposium on User Interface Software and Technology',
        published: true,
        paper: 'lee2022lv.pdf',
    },
    lee2022methodAndApparatus: {
        year: 2022,
        title:
            'Method and apparatus for context-adaptive personalized psychological state sampling for wearable device',
        authors: [
            "Uichin Lee",
            "Cheulyoung Park",
            "Soowon Kang",
            "Narae Cha"
        ],
        venue_alias: 'Google Patents',
        venue: 'Google Patents',
        published: true,
        paper: 'lee2022method.pdf',
    },
    lee2022methodAndSystem: {
        year: 2022,
        title:
            'Method and system for distraction management of context-aware rule-based smart device',
        authors: [
            "Uichin Lee",
            "Inyeop Kim"
        ],
        venue_alias: 'Google Patents',
        venue: 'Google Patents',
        published: true,
        paper: 'lee2022method.pdf',
    },
    choi2022facilitating: {
        year: 2022,
        title: 'Facilitating instant interactions for stressful experiences sharing and peer support',
        authors: [
            "Ryuhaerang Choi",
            "Chanwoo Yun",
            "Hyunsung Cho",
            "Hwajung Hong",
            "Uichin Lee",
            "Sung-ju Lee"
        ],
        venue_alias: 'MobiSys',
        venue: 'Proceedings of the 20th Annual International Conference on Mobile Systems, Applications and Services',
        published: true,
        paper: 'choi2022facilitating.pdf',
    },
    lee2022beyond: {
        year: 2022,
        title:
            'Beyond pathogen filtration: Possibility of smart masks as wearable devices for personal and group health and safety management',
        authors: [
            "Peter Lee",
            "Heepyung Kim",
            "Yongshin Kim",
            "Woohyeok Choi",
            "M. Sami Zitouni",
            "Ahsan Khandoker",
            "Herbert F. Jelinek",
            "Leontios Hadjileontiadis",
            "Uichin Lee",
            "Yong Jeong"
        ],
        venue_alias: 'JMU',
        venue: 'JMIR mHealth and uHealth',
        published: true,
        paper: 'lee2022beyond.pdf',
    },
    salman2022darcas: {
        year: 2022,
        title:
            'DARCAS: Dynamic Association Regulator Considering Airtime Over SDN-Enabled Framework',
        authors: [
            "Muhammad Salman",
            "Jin-ho Son",
            "Dong-wan Choi",
            "Uichin Lee",
            "Youngtae Noh"
        ],
        venue_alias: 'IEEE',
        venue: 'IEEE Internet of Things Journal',
        published: true,
        paper: 'salman2022darcas.pdf',
    },
    kim2022social: {
        year: 2022,
        title: 'Social-Spiritual Face: Designing Social Reading Support for Spiritual Well-being',
        authors: [
            'Inyeop Kim',
            'Minsam Ko',
            'Joonyoung Park',
            'Sung Wook Moon',
            'Gyuwon Jung',
            'Youn-kyung Lim',
            'Uichin Lee',
        ],
        venue_alias: 'CSCW',
        venue:
            "Proceedings of the ACM on Human-Computer Interaction (PACM-HCI) / CSCW, Volume 6, CSCW2, November'22",
        published: true,
        video: 'https://youtu.be/Ohd-YyQ8tl4',
        paper: 'kim2022social.pdf',
    },
    lee2022adverse: {
        year: 2022,
        title: 'Editorial: Adverse Health Consequences of Excessive Smartphone Usage',
        authors: ['Uichin Lee', 'Paul H. Lee'],
        venue_alias: 'Frontiers Public Health',
        venue: 'Volume II, Frontiers Public Health, 09 August 2022 Sec. Digital Public Health',
        paper: 'lee2022adverse.pdf',
    },
    lee2021systematic: {
        year: 2022,
        title: 'A Systematic Survey on Android API Usage for Data-Driven Analytics with Smartphones',
        authors: ['Hansoo Lee', 'Joonyoung Park', 'Uichin Lee'],
        venue_alias: 'ACM CSUR',
        venue: 'ACM Computing Surveys (CSUR)',
        paper: 'lee2021systematic.pdf',
    },
    lee2022understanding: {
        year: 2022,
        title:
            'Understanding Perceived Benefits and Privacy Risks in Open Dataset Collection for Mobile Affective Computing',
        authors: ['Hyunsoo Lee', 'Soowon Kang', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2022 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 6, Issue 2, July 2022',
        paper: 'lee2022understanding.pdf',
        slide: 'lee2022understanding.pdf'
    },
    salman2022csi: {
        year: 2022,
        title:
            'CSI:DeSpy- Enabling Effortless Spy Camera Detection via Passive Sensing of User Activities and Bitrate Variations',
        authors: ['Muhammad Salman', 'Nguyen Dao', 'Uichin Lee', 'Youngtae Noh'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2022 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 6, Issue 2, July 2022',
        paper: 'salman2022csi.pdf',
    },
    jung2022caremouse: {
        year: 2022,
        title:
            'CareMouse: An Interactive Mouse System that Supports Wrist Stretching Exercises in the Workplace',
        authors: ['Gyuwon Jung', 'Youwon Shin', 'Jieun Lim', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            "CHI EA '22: Extended Abstracts of the 2022 CHI Conference on Human Factors in Computing Systems",
        paper: 'jung2022caremouse.pdf',
        slide: 'jung2022caremouse.pdf',
        video: 'https://www.youtube.com/watch?v=lUj4s3ITmxs',
    },
    kang2022understanding: {
        year: 2022,
        title: 'Understanding Emotion Changes in Mobile Experience Sampling',
        authors: ['Soowon Kang', 'Cheul Young Park', 'Narae Cha', 'Auk Kim', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue: 'CHI Conference on Human Factors in Computing Systems',
        paper: 'kang2022understanding.pdf',
        video: 'https://youtu.be/CCw1uEOuhZM',
        slide: 'kang2022understanding.pdf'
    },
    choi2022s: {
        year: 2022,
        title:
            "It’s not wrong, but I'm quite disappointed: Toward an Inclusive Algorithmic Experience for Content Creators with Disabilities",
        authors: ['Dasom Choi', 'Uichin Lee', 'Hwajung Hong'],
        venue_alias: 'CHI',
        venue: 'CHI Conference on Human Factors in Computing Systems',
        paper: 'choi2022s.pdf',
    },
    zitouni2022privacy: {
        year: 2022,
        title: 'Privacy Aware Affective State Recognition from Visual Data',
        authors: [
            'M Sami Zitouni',
            'Peter Lee',
            'Uichin Lee',
            'Leontios Hadjileontiadis',
            'Ahsan Khandoker',
        ],
        venue_alias: 'IEEE',
        venue: 'IEEE Access 07 April 2022',
        paper: 'zitouni2022privacy.pdf',
    },
    al2022successfully: {
        year: 2022,
        title:
            'Successfully implementing digital health to ensure future global health security during pandemics: a consensus statement',
        authors: [
            'Bandar Al Knawy',
            'Mollie Marian McKillop',
            'Joud Abduljawad',
            'Sasu Tarkoma',
            'Mahmood Adil',
            'Louise Schaper',
            'Adam Chee',
            'David W Bates',
            'Michael Klag',
            'Uichin Lee',
            'Zisis Kozlakidis',
            'George Crooks',
            'Kyu Rhee',
        ],
        venue_alias: 'JAMA',
        venue: 'JAMA Network Open 2022;5(2):e220214',
        paper: 'al2022successfully.pdf',
    },
    kim2021poster: {
        year: 2021,
        title:
            'Poster: Exploring User Contexts and Needs for Context-aware Smartphone Distraction Management',
        authors: [
            'Inyeop Kim',
            'Hwarang Goh',
            'Youngtae Noh',
            'Uichin Lee'
        ],
        venue_alias: 'Ubicomp',
        venue: 'Adjunct Proceedings of the 2021 ACM International Joint Conference on Pervasive and Ubiquitous Computing and Proceedings of the 2021 ACM International Symposium on Wearable Computers',
        paper: 'kim2021poster.pdf',
    },
    kang2021poster: {
        year: 2021,
        title:
            'Poster: Toward Context-aware Proactive Conversation for Smart Speakerst',
        authors: [
            'Soowon Kang',
            'Heepyung Kim',
            'Youngtae Noh',
            'Uichin Lee'
        ],
        venue_alias: 'Ubicomp',
        venue: 'Adjunct Proceedings of the 2021 ACM International Joint Conference on Pervasive and Ubiquitous Computing and Proceedings of the 2021 ACM International Symposium on Wearable Computers',
        paper: 'kang2021poster.pdf',
    },
    kim2021causal: {
        year: 2021,
        title:
            'Causal Analysis of Observational Mobile Sensor Data: A Comparative Study',
        authors: [
            'Yongshin Kim',
            'Panyu Zhang',
            'Gyuwon Jung',
            'Heepyung Kim',
            'Uichin Lee'
        ],
        venue_alias: '한국정보과학회',
        venue: '한국정보과학회 학술발표논문집',
        slide: 'kim2021causal.pdf',
        paper: 'kim2021causal.pdf',
    },
    kim2021cheersbot: {
        year: 2021,
        title: "Cheersbot: 외로움 개선을 위한 물리적 / 청각적 상호작용을 제공하는 술친구 소셜 로봇",
        authors: ["김채원", "정유경", "정규원", "우운택", "이의진"],
        venue_alias: "HCI Korea",
        venue: "한국HCI학회 학술대회, 2021",
        published: true,
        paper: "kim2021cheersbot.pdf"
    },
    nasrat2021emotion: {
        year: 2021,
        title:
            'Emotion Recognition in the Wild from Long-term Heart Rate Recording using Wearable Sensor and Deep Learning Ensemble Classification',
        authors: [
            'Sara A Nasrat',
            'Uichin Lee',
            'M Sami Zitouni',
            'Ahsan H Khandoker',
            'Soowon Kang',
            'Herbert F Jelinek',
        ],
        venue_alias: 'BIBM',
        venue: '2021 IEEE International Conference on Bioinformatics and Biomedicine (BIBM)',
        paper: 'nasrat2021emotion.pdf',
    },
    zitouni2021arousal: {
        year: 2021,
        title:
            'Arousal-Valence Classification from Peripheral Physiological Signals Using Long Short-Term Memory Networks',
        authors: [
            'M Sami Zitouni',
            'Cheul Young Park',
            'Uichin Lee',
            'Leontios Hadjileontiadis',
            'Ahsan Khandoker',
        ],
        venue_alias: 'EMBC',
        venue:
            '2021 43rd Annual International Conference of the IEEE Engineering in Medicine & Biology Society (EMBC)',
        paper: 'zitouni2021arousal.pdf',
    },
    lee2021dynamic: {
        year: 2021,
        title: 'Dynamic Consent for Sensor-Driven Research',
        authors: ['Hyunsoo Lee', 'Uichin Lee'],
        venue_alias: 'ICMU',
        venue:
            'ICMU 2021, The 13th International Conference on Mobile Computing and Ubiquitous Networking, November 17-19th, 2021, Tokyo, Japan',
        paper: 'lee2021dynamic.pdf',
    },
    kim2021utilitarian: {
        year: 2021,
        title:
            'Utilitarian or Relational? Exploring Indicators of User Orientation Towards Intelligent Agents',
        authors: ['Hankyung Kim', 'Hoyeon Nam', 'Uichin Lee', 'Youn-kyung Lim'],
        venue_alias: 'HCII',
        venue:
            'HCI International 2021, International Conference on Human-Computer Interaction, 2021/7/24',
        paper: 'kim2021utilitarian.pdf',
    },
    lee2021benefits: {
        year: 2021,
        title: 'Benefits of Mobile Contact Tracing on COVID-19: Tracing Capacity Perspectives',
        authors: ['Uichin Lee', 'Auk Kim'],
        venue_alias: 'Frontiers Public Health',
        venue:
            'Frontiers in Public Health (Digital Public Health Section), 18 March 2021 (to appear)',
        // cannot download pdf file
        paper: 'lee2021benefits.pdf',
    },
    jung2021good: {
        year: 2021,
        title: '“Good Enough!”: Flexible Goal Achievement with Margin-based Outcome Evaluation',
        authors: ['Gyuwon Jung', 'Jio Oh', 'Youjin Jung', 'Juho Sun', 'Ha-Kyung Kong', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue: 'Proceedings of the 2021 CHI Conference on Human Factors in Computing Systems',
        paper: 'jung2021good.pdf',
        video: 'https://www.youtube.com/watch?v=mxMoqi58A4I&feature=youtu.be',
    },
    lee2021sticky: {
        year: 2021,
        title: 'Sticky Goals: Understanding Goal Commitments for Behavioral Changes in the Wild',
        authors: ['Hyunsoo Lee', 'Auk Kim', 'Hwajung Hong', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue: 'Proceedings of the 2021 CHI Conference on Human Factors in Computing Systems',
        paper: 'lee2021sticky.pdf',
        video: 'https://www.youtube.com/watch?v=mSCXgOceCpA&ab_channel=ACMSIGCHI',
        slide: 'lee2021sticky.pdf'
    },
    park2021goldentime: {
        year: 2021,
        title:
            'GoldenTime: Exploring System-Driven Timeboxing and Micro-Financial Incentives for Self-Regulated Phone Use',
        authors: ['Joonyoung Park', 'Hyunsoo Lee', 'Sangkeun Park', 'Kyong-Mee Chung', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue: 'Proceedings of the 2021 CHI Conference on Human Factors in Computing Systems',
        paper: 'park2021goldentime.pdf',
        slide: 'park2021goldentime.pdf',
        video: 'https://www.youtube.com/watch?v=oVSeVFfBhQE',
    },
    kim2021beneficial: {
        year: 2021,
        title:
            'Beneficial Neglect: Instant Message Notification Handling Behaviors and Academic Performance',
        authors: ['Minhyung Kim', 'Inyeop Kim', 'Uichin Lee ACM'],
        venue_alias: 'IMWUT',
        venue:
            'Ubicomp 2021 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 5, Issue 1, March 2021',
        paper: 'kim2021beneficial.pdf',
    },
    samson2020asian: {
        year: 2020,
        title:
            'Asian CHI symposium: HCI research from Asia and on Asian contexts and cultures',
        authors: [
            'Briane Paul V. Samson',
            'Suleman Shahid',
            'Akihiro Matsufuji',
            'Chat Wacharamanotham',
            'Toni-Jan Keith P. Monserrat',
            'Keyur Sorathia',
            'Masitah Ghazali',
            'Shio Miyafuji',
            'Nova Ahmed',
            'Kazuyuki Fujita',
            'A. B. M. Alim Al Islam',
            'Eunice Sari',
            'Murni Mahmud',
            'Adi Tedjasaputra',
            'Juho Kim',
            'Uichin Lee',
            'Thippaya Chintakovid',
            'Sheng-Ming Wang',
            'Zhengjie Liu',
            'Xiangmin Fan',
            'Ellen Yi-Luen Do',
            'Yoshifumi Kitamura',
            'Simon Tangi Perrault',
            'Bing-Yu Chen'
        ],
        venue_alias: 'CHI',
        venue:
            "Extended Abstracts of the 2020 CHI Conference on Human Factors in Computing Systems",
        paper: 'samson2020asian.pdf',
    },
    kim2020understanding: {
        year: 2020,
        title:
            'Understanding User Contexts and Coping Strategies for Context-aware Phone Distraction Management System Design',
        authors: ['Inyeop Kim', 'Hwarang Goh', 'Nematjon Narziev', 'Youngtae Noh', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2021/ Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT) Volume 4, Issue 4, Dec. 2020',
        paper: 'kim2020understanding.pdf',
    },
    lee2021validation: {
        year: 2020,
        title:
            'Validation of self-reported smartphone usage against objectively-measured smartphone usage in Hong Kong Chinese adolescents and young adults',
        authors: ['Paul H Lee', 'Andy C. Y. Tse', 'Cynthia S. T. Wu', 'Yim Wah Mak', 'Uichin Lee'],
        venue_alias: 'PI',
        venue: 'Psychiatry Investigation 2021 Feb',
        paper: 'lee2021validation.pdf',
    },
    al2020riyadh: {
        year: 2020,
        title: 'The Riyadh Declaration: the role of digital health in fighting pandemics',
        authors: [
            'Bandar Al Knawy',
            'Mahmood Adil',
            'George Crooks',
            'Kyu Rhee',
            'David Bates',
            'Hani Jokhdar',
            'Michael Klag',
            'Uichin Lee',
            'Ali H Mokdad',
            'Louise Schaper',
            'Raed Al Hazme',
            'Ali M Al Khathaami',
            'Joud Abduljawad',
        ],
        venue_alias: 'Lancet',
        venue: 'The Lancet 2020 Volume 396, Issue 10262',
        paper: 'al2020riyadh.pdf',
    },
    lee2021temporal: {
        year: 2020,
        title:
            'Temporal association between objectively measured smartphone usage, sleep quality and physical activity among Chinese adolescents and young adults',
        authors: ['Paul H. Lee', 'Andy C. Y. Tse', 'Cynthia S. T. Wu', 'Yim Wah Mak', 'Uichin Lee'],
        venue_alias: 'JSR',
        venue: 'Journal of Sleep Research Oct. 2020',
        paper: 'lee2021temporal.pdf',
    },
    murray2020digital: {
        year: 2020,
        title: 'Digital Public Health and COVID-19',
        authors: ['Christopher J L Murray', 'Nurah Maziad S Alamro', 'Hee Hwang', 'Uichin Lee'],
        venue_alias: 'Lancet',
        venue: 'Lancet Public Health Volume 5, Issue 9, E469-E470, Sept. 2020',
        paper: 'murray2020digital.pdf',
    },
    park2020k: {
        year: 2020,
        title:
            'K-EmoCon, a multimodal sensor dataset for continuous emotion recognition in naturalistic conversations',
        authors: [
            'Cheul Young Park',
            'Narae Cha',
            'Soowon Kang',
            'Auk Kim',
            'Ahsan Habib Khandoker',
            'Leontios Hadjileontiadis',
            'Alice Oh',
            'Yong Jeong',
            'and Uichin Lee',
        ],
        venue_alias: 'Scientific Data',
        venue: 'Scientific Data Volume 7, No. 293, 2020',
        paper: 'park2020k.pdf',
    },
    cha2020hello: {
        year: 2020,
        title:
            'Hello there! Is now a good time to talk?: Opportune Moments for Proactive Conversational Interaction with Smart Speakers',
        authors: [
            'Narae Cha',
            'Auk Kim',
            'Cheul Young Park',
            'Soowon Kang',
            'Mingyu Park',
            'Jae-Gil Lee',
            'Sangsu Lee',
            'Uichin Lee',
        ],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2021/ Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT) Vol. 4, No. 3, Article 74, Sept. 2020',
        paper: 'cha2020hello.pdf',
        video: 'https://www.youtube.com/watch?v=AA8CTi2hEf0',
    },
    jung2020too: {
        year: 2020,
        title:
            'Too Much Information: Assessing Privacy Risks of Contact Trace Data Disclosure on People With COVID-19 in South Korea',
        authors: ['Gyuwon Jung', 'Hyunsoo Lee', 'Auk Kim', 'Uichin Lee'],
        venue_alias: 'FrontPubHealth',
        venue: 'Frontiers in Public Health 2020',
        paper: 'jung2020too.pdf',
    },
    kim2020dashcam: {
        year: 2020,
        title: 'Dashcam Witness: Video Sharing Motives and Privacy Concerns across Different Nations',
        authors: ['Joohyun Kim', 'Sangkeun Park', 'Uichin Lee'],
        venue_alias: 'IEEE',
        venue: 'IEEE Access 2020',
        paper: 'kim2020dashcam.pdf',
        slide: 'kim2020dashcam.pdf',
    },
    balili2020termball: {
        year: 2020,
        title:
            'TermBall: Tracking and Predicting Evolution Types of Research Topics by Using Knowledge Structures in Scholarly Big Data',
        authors: ['Christine Balili', 'Uichin Lee', 'Aviv Segev', 'Jaejeung Kim', 'Minsam Ko'],
        venue_alias: 'IEEE',
        venue: 'IEEE Access 2020',
        paper: 'balili2020termball.pdf',
    },
    park2020cognitive: {
        year: 2020,
        title:
            'Cognitive States Matter: Design Guidelines for Driving Situation Awareness in Smart Vehicles',
        authors: ['Daehee Park', 'Wan Chul Yoon', 'Uichin Lee'],
        venue_alias: 'Sensors',
        venue: 'Sensors 2020, 20(10), 2978',
        paper: 'park2020cognitive.pdf',
    },
    rhim2020tracking: {
        year: 2020,
        title: 'Tracking and Modeling Subjective Well-Being Using Smartphone-Based Digital Phenotype',
        authors: ['Soyoung Rhim', 'Uichin Lee', 'Kyungsik Han'],
        venue_alias: 'UMAP',
        venue:
            "ACM UMAP'20 / Conference on User Modeling, Adaptation and Personalization , July 14-17, 2020, Genoa, Italy",
        paper: 'rhim2020tracking.pdf',
    },
    lee2020privacy: {
        year: 2020,
        title: 'Privacy Concerns of Digital Phenotyping for Older Adults with Mental Health Issues',
        authors: ['Hyunsoo Lee', 'Uichin Lee'],
        venue_alias: 'CHI Workshop',
        venue: 'ACM CHI 2020 Networked Privacy Workshop, April 25-30, 2020',
        paper: 'lee2020privacy.pdf',
    },
    sun2020beactive: {
        year: 2020,
        title:
            'BeActive: Encouraging Physical Activities with Just-in-time Health Intervention and Micro Financial Incentives',
        authors: [
            'Juho Sun',
            'Sangkeun Park',
            'Gyuwon Jung',
            'Yong Jeong',
            'Uichin Lee',
            'Kyong-Mee Chung',
            'Changseok Lee',
            'Heewon Kim',
            'Suhyon Ahn',
            'Ahsan Khandoker',
            'Leontios Hadjileontiadis',
        ],
        venue_alias: 'AsianCHI',
        venue:
            "AsianCHI '20: Proceedings of the 2020 Symposium on Emerging Research from Asia and on Asian Contexts and Cultures, April 2020 Pages 17-20",
        paper: 'sun2020beactive.pdf',
    },
    oh2020scriptfree: {
        year: 2020,
        title:
            'ScriptFree: Designing Speech Preparation Systems with Adaptive Visual Reliance Control on Script',
        authors: ['Jeungmin Oh', 'Darren Edge', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            "CHI EA '20: Extended Abstracts of the 2020 CHI Conference on Human Factors in Computing Systems, April 2020",
        paper: 'oh2020scriptfree.pdf',
    },
    kim2020interruptibility: {
        year: 2020,
        title:
            'Interruptibility for In-vehicle Multitasking: Influence of Voice Task Demands and Adaptive Behaviors',
        authors: ['Auk Kim', 'Jung-Mi Park', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2020 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 4, Issue 1, March 2020',
        paper: 'kim2020interruptibility.pdf',
    },
    lee2020objectively: {
        year: 2020,
        title: 'Objectively-Measured Smartphone Usage, Sleep Quality, and Physical Activity Among Chinese Adolescents and Young Adults',
        venue: "ResearchSquare",
        venue_alias: "ResearchSquare",
        authors: ['Peter H Lee', 'Andy C. Y. Tse', 'Cynthia S. T. Wu', 'Yim Wah Mak', 'Uichin Lee'],
        paper: 'lee2020objectively.pdf',
    },
    lee2019maximizing: {
        year: 2019,
        title:
            'Maximizing MapReduce Job Speed and Reliability in the Mobile Cloud by Optimizing Task Allocation',
        authors: ['Jin-woo Lee', 'Gwangseon Jang', 'Hohyun Jung', 'Jae-Gil Lee', 'Uichin Lee'],
        venue_alias: 'PMC',
        venue:
            'Pervasive and Mobile Computing Pervasive and Mobile Computing Volume 60, November 2019, 101082',
        paper: 'lee2019maximizing.pdf',
    },
    kim2019predicting: {
        year: 2019,
        title: 'Predicting Opportune Moments for In-vehicle Proactive Speech Services',
        authors: ['Auk Kim', 'Woohyeok Choi', 'Jungmi Park', 'Kyeyoon Kim', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue:
            'Adjunct Proceedings of the 2019 ACM International Joint Conference on Pervasive and Ubiquitous Computing and Proceedings of the 2019 ACM International Symposium on Wearable Computers',
        paper: 'kim2019predicting.pdf',
    },
    lee2019pass: {
        year: 2019,
        title:
            'PASS: Reducing Redundant Notifications between a Smartphone and a Smartwatch for Energy Saving',
        authors: ['Jemin Lee', 'Uichin Lee', 'Hyungshin Kim'],
        venue_alias: 'IEEE',
        venue: 'IEEE Transactions on Mobile Computing 2019',
        paper: 'lee2019pass.pdf',
    },
    kim2019understanding: {
        year: 2019,
        title: 'Understanding Smartphone Usage in College Classrooms: A Long-term Measurement Study',
        authors: [
            'Inyeop Kim',
            'Rihun Kim',
            'Heepyung Kim',
            'Duyeon Kim',
            'Kyungsik Han',
            'Paul H Lee',
            'Gloria Mark',
            'and Uichin Lee',
        ],
        venue_alias: 'Computers & Education',
        venue: 'Computers & Education Volume 141, November 2019, 103611',
        paper: 'kim2019understanding.pdf',
    },
    choi2019multi: {
        year: 2019,
        title: 'Multi-Stage Receptivity Model for Mobile Just-In-Time Health Intervention',
        authors: ['Woohyeok Choi', 'Sangkeun Park', 'Duyeon Kim', 'Youn-kyung Lim', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2019 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 3, Issue 2, Article 39, June 2019',
        paper: 'choi2019multi.pdf',
    },
    kim2019lockntype: {
        year: 2019,
        title: 'LocknType: Lockout Task Intervention for Discouraging Smartphone App Use',
        authors: ['Jaejeung Kim', 'Joonyoung Park', 'Hyunsoo Lee', 'Minsam Ko', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2019 CHI conference on human factors in computing systems, May 2019',
        paper: 'kim2019lockntype.pdf',
        slide: 'kim2019lockntype.pdf',
    },

    //////////////
    // HS start //
    //////////////

    lee2019commitment: {
        year: 2019,
        title: 'Commitment devices in online behavior change support systems',
        authors: ['Hyunsoo Lee', 'Uichin Lee', 'Hwajung Hong'],
        venue_alias: 'AsianCHI',
        venue:
            "AsianHCI '19: Proceedings of Asian CHI Symposium 2019: Emerging HCI Research Collection, May 2019",
        paper: 'lee2019commitment.pdf',
    },
    shin2019slow: {
        year: 2019,
        title: 'Slow Robots for Unobtrusive Posture Correction',
        authors: [
            'Joon-Gi Shin',
            'Eiji Onchi',
            'Maria Jose Reyes',
            'Junbong Song',
            'Uichin Lee',
            'Seung-Hee Lee',
            'Daniel Saakes',
        ],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2019 CHI Conference on Human Factors in Computing Systems, May 2019',
        paper: 'shin2019slow.pdf',
        video: 'https://www.youtube.com/watch?v=0iQ6hzU_ct0',
    },
    lee2019interaction: {
        year: 2019,
        title: 'Interaction Restraint Framework for Digital Wellbeing',
        authors: ['Uichin Lee', 'Jaejeung Kim'],
        venue_alias: 'CHI Workshop',
        venue:
            'ACM CHI Workshop: Designing for Digital Wellbeing , Glasgow, Scotland UK, May 4, 2019',
        paper: 'lee2019interaction.pdf',
        slide: 'lee2019interaction.pdf'
    },
    oh2019wake: {
        year: 2019,
        title: 'Wake-Up Task: Understanding Users in Task-based Mobile Alarm App',
        authors: [
            'Kyue Taek Oh',
            'Jaemyung Shin',
            'Jaejeung Kim',
            'In Suk Song',
            'Uichin Lee',
            'Eunil Park',
            'Jinyoung Han',
            'Minsam Ko',
        ],
        venue_alias: 'CHI EA',
        venue:
            "CHI EA '19: Extended Abstracts of the 2019 CHI Conference on Human Factors in Computing Systems, May 2019",
        paper: 'oh2019wake.pdf',
    },
    kim2019goalkeeper: {
        year: 2019,
        title: 'GoalKeeper: Exploring Interaction Lockout Mechanisms for Regulating Smartphone Use',
        authors: ['Jaejeung Kim', 'Hayoung Jung', 'Minsam Ko', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2019 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 3, Issue 1, Mar. 2019',
        paper: 'kim2019goalkeeper.pdf',
    },
    kang2019fire: {
        year: 2019,
        title: 'Fire in Your Hands: Understanding Thermal Behavior of Smartphones',
        authors: [
            'Soowon Kang',
            'Hyeonwoo Choi',
            'Sooyoung Park',
            'Chunjong Park',
            'Jemin Lee',
            'Uichin Lee',
            'Sung-Ju Lee',
        ],
        venue_alias: 'MobiCom',
        venue:
            "MobiCom '19: The 25th Annual International Conference on Mobile Computing and Networking, August 2019",
        paper: 'kang2019fire.pdf',
        video: 'https://youtu.be/yLNF0-S5aLE',
    },
    kim2018interrupting: {
        year: 2019,
        title:
            'Interrupting Drivers for Interactions: Predicting Opportune Moments for In-vehicle Proactive Auditory-verbal Tasks',
        authors: ['Auk Kim', 'Woohyeok Choi', 'Jung-Mi Park', 'Kyeyoon Kim', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2019 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 2, Issue 4, Dec. 2018',
        paper: 'kim2018interrupting.pdf',
    },
    이의진2019긍정: {
        year: 2019,
        title: '긍정 컴퓨팅을 위한 설득적 인터랙션 제한 시스템',
        authors: ['이의진', '김재정'],
        venue_alias: 'HCI Korea',
        venue: 'HCI Korea 2019, 2019.2.13-16',
        paper: '이의진2019긍정.pdf',
    },
    kang2018understanding: {
        year: 2018,
        title: "Understanding Customers'Interests in the Wild",
        authors: ['Soowon Kang', 'Auk Kim', 'Jemin Lee', 'Ikhee Shin', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue:
            'Proceedings of the 2018 ACM International Joint Conference and 2018 International Symposium on Pervasive and Ubiquitous Computing and Wearable Computers',
        paper: 'kang2018understanding.pdf',
    },
    cha2018hellobot: {
        year: 2018,
        title: 'HelloBot: Facilitating Social Inclusion with an Interactive Greeting Robot',
        authors: ['Narae Cha', 'Inyeop Kim', 'Mingyu Park', 'Auk Kim', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue:
            'Proceedings of the 2018 ACM International Joint Conference and 2018 International Symposium on Pervasive and Ubiquitous Computing and Wearable Computers',
        paper: 'cha2018hellobot.pdf',
    },
    kim2018exploring: {
        year: 2018,
        title: 'Exploring Context-aware Proactive Blocking for Distraction Management',
        authors: ['Inyeop Kim', 'Uichin Lee', 'Narae Cha'],
        venue_alias: 'Ubicomp',
        venue:
            'Proceedings of the 2018 ACM International Joint Conference and 2018 International Symposium on Pervasive and Ubiquitous Computing and Wearable Computers',
        paper: 'kim2018exploring.pdf',
    },
    park2018campuswatch: {
        year: 2018,
        title: 'CampusWatch: Exploring Communitysourced Patrolling with Pervasive Mobile Technology',
        authors: ['Sangkeun Park', 'Sujin Kown', 'Uichin Lee'],
        venue_alias: 'PACM-HCI',
        venue:
            'Proceedings of the ACM on Human-Computer Interaction, Vol. 2, Issue CSCW, Article 134, Nov. 2018',
        paper: 'park2018campuswatch.pdf',
    },
    cha2018complex: {
        year: 2018,
        title: 'Complex and Ambiguous: Understanding Sticker Misinterpretations in Instant Messaging',
        authors: ['Yoonjeong Cha', 'Jongwon Kim', 'Sangkeun Park', 'Mun Yong Yi', 'Uichin Lee'],
        venue_alias: 'PACM-HCI',
        venue:
            'Proceedings of the ACM on Human-Computer Interaction, Vol. 2, Issue CSCW, Article 30, Nov. 2018',
        paper: 'cha2018complex.pdf',
    },
    park2018localness: {
        year: 2018,
        title: 'Localness of Location-based Knowledge Sharing: A Study of Naver KiN “Here”',
        authors: ['Sangkeun Park', 'Mark S. Ackerman', 'Uichin Lee'],
        venue_alias: 'TWEB',
        venue: 'ACM Transactions on the Web, Volume 12, Issue 3, August 2018',
        paper: 'park2018localness.pdf',
    },
    lee2019intelligent: {
        year: 2018,
        title:
            'Intelligent Positive Computing with Mobile, Wearable, and IoT Devices: Literature Review and Research Directions',
        authors: [
            'Uichin Lee',
            'Kyungsik Han',
            'Hyunsung Cho',
            'Kyong-Mee Chung',
            'Hwajung Hong',
            'Sung-Ju Lee',
            'Youngtae Noh',
            'Sooyoung Park',
            'John M. Carroll',
        ],
        venue_alias: 'Ad Hoc Networks',
        venue: 'Ad Hoc Networks, Volume 83, Pages 8-24, February 2019',
        paper: 'lee2019intelligent.pdf',
    },
    han2019optical: {
        year: 2018,
        title:
            'Optical-acoustic Hybrid Network Toward Real-time Video Streaming for Mobile Underwater Sensors',
        authors: ['Sungwon Han', 'Youngtae Noh', 'Uichin Lee', 'Mario Gerla'],
        venue_alias: 'Ad Hoc Networks',
        venue: 'Ad Hoc Networks, Volume 83, Pages 1-7, February 2019',
        paper: 'han2019optical.pdf',
    },
    park2018interaction: {
        year: 2018,
        title:
            'Interaction Restraint: Enforcing Adaptive Cognitive Tasks to Restrain Problematic User Interaction',
        authors: ['Joonyoung Park', 'Jin Yong Sim', 'Jaejeung Kim', 'Mun Yong Yi', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Extended Abstracts of the 2018 CHI Conference on Human Factors in Computing Systems, April 2018',
        paper: 'park2018interaction.pdf',
        slide: 'park2018interaction.pdf',
        video: 'https://drive.google.com/file/d/1S4_C4uozwrfX9yvtBXcCj7u0y33LHzHU/view?usp=sharing',
    },
    shin2018actuating: {
        year: 2018,
        title: 'Actuating a Monitor for Posture Changes',
        authors: ['Joon Gi Shin', 'Woohyeok Choi', 'Uichin Lee', 'Daniel Saakes'],
        venue_alias: 'CHI EA',
        venue:
            'Extended Abstracts of the 2018 CHI Conference on Human Factors in Computing Systems, April 2018',
        paper: 'shin2018actuating.pdf',
    },
    shin2018tnt: {
        year: 2018,
        title: 'TNT: Exploring Pseudo Social Reminding for Effective Task Management',
        authors: ['Wonyoung Shin', 'Soowon Kang', 'Inyeop Kim', 'Mun Yong Yi', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Extended Abstracts of the 2018 CHI Conference on Human Factors in Computing Systems, April 2018',
        paper: 'shin2018tnt.pdf',
    },
    lee2018flower: {
        year: 2017,
        title: 'Flower-Pop: Facilitating Casual Group Conversations With Multiple Mobile Devices',
        authors: [
            'Moon-Hwan Lee',
            'Yea-Kyung Row',
            'Oosung Son',
            'Uichin Lee',
            'Jaejeung Kim',
            'Jungi Jeong',
            'Seungryoul Maeng',
            'Tek-Jin Nam',
        ],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2018 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 1, Issue 4, Dec. 2017',
        paper: 'lee2018flower.pdf',
        slide: 'lee2018flower.pdf',
    },
    balili2017tracking: {
        year: 2017,
        title: 'Tracking and Predicting the Evolution of Research Topics in Scientific Literature',
        authors: ['Christine Balili', 'Aviv Segev', 'Uichin Lee'],
        venue_alias: 'IEEE',
        venue: 'IEEE BigData 2017, Dec 11-14, 2017 Boston, MA, USA',
        paper: 'balili2017tracking.pdf',
    },
    kim2017technology: {
        year: 2017,
        title:
            'Technology Supported Behavior Restriction for Mitigating Self-Interruptions in Multi-device Environments',
        authors: ['Jaejeung Kim', 'Chiwoo Cho', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2017 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 1, Issue 3, Sept. 2017',
        paper: 'kim2017technology.pdf',
        slide: "kim2017technology.pdf",
    },
    kim2017let: {
        year: 2017,
        title: "Let's FOCUS: Mitigating Mobile Phone Use in College Classrooms",
        authors: ['Inyeop Kim', 'Gyuwon Jung', 'Hayoung Jung', 'Minsam Ko', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2017 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 1, Issue 3, Sept. 2017',
        paper: 'kim2017let.pdf',
        slide: 'kim2017let.pdf',
        video: 'https://www.youtube.com/watch?v=orm1dFj3RY0',
    },
    kim2017trailsense: {
        year: 2017,
        title:
            'TrailSense: A Crowdsensing System for Detecting Risky Mountain Trail Segments with Walking Pattern Analysis',
        authors: ['Keunseo Kim', 'Hengameh Zabihi', 'Heeyoung Kim', 'Uichin Lee'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2017 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 1, Issue 3, Sept. 2017',
        paper: 'kim2017trailsense.pdf',
        slide: "kim2017trailsense.pdf",
    },
    jeong2017smartwatch: {
        year: 2017,
        title: 'Smartwatch Wearing Behavior Analysis: A Longitudinal Study',
        authors: ['Hayeon Jeong', 'Heepyung Kim', 'Rihun Kim', 'Uichin Lee', 'Yong Jeong'],
        venue_alias: 'IMWUT',
        venue:
            'ACM Ubicomp 2017 / Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT), Volume 1, Issue 3, Sept. 2017',
        paper: 'jeong2017smartwatch.pdf',
        slide: "jeong2017smartwatch.pdf",
    },
    oh2017understanding: {
        year: 2017,
        title: 'Understanding Mobile Document Capture and Correcting Orientation Errors',
        authors: [
            'Jeungmin Oh',
            'Joohyun Kim',
            'Myungjoon Kim',
            'Woohyeok Choi',
            'SangJeong Lee',
            'Uichin Lee',
        ],
        venue_alias: 'IJHCS',
        venue:
            'International Journal of Human-Computer Studies, Volume 104, Pages 64-79, August 2017',
        paper: 'oh2017understanding.pdf',
    },
    park2017virtual: {
        year: 2017,
        title: 'Virtual Figure Model Crafting with VR HMD and Leap Motion',
        authors: ['Gangrae Park', 'Hyunmin Choi', 'Uichin Lee', 'Seongah Chin'],
        venue_alias: 'The Imaging Science Journal',
        venue: 'The Imaging Science Journal, Pages 358-370, Sept. 2017',
        paper: 'park2017virtual.pdf',
    },
    chin2017watchout: {
        year: 2017,
        title: 'WatchOut: Facilitating Safe Driving Behaviors with Social Support',
        authors: ['Hyojin Chin', 'Hengameh Zabihi', 'Sangkeun Park', 'Mun Yong Yi', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2017 CHI Conference Extended Abstracts on Human Factors in Computing Systems, May 2017',
        paper: 'chin2017watchout.pdf',
        slide: 'chin2017watchout.pdf',
    },
    kim2017sensors: {
        year: 2017,
        title: 'Sensors Know Which Photos Are Memorable',
        authors: [
            'Soyoung Kim',
            'Kalenzi Asio Evelyn Patra',
            'Auk Kim',
            'Kun-Pyo Lee',
            'Aviv Segev',
            'Uichin Lee',
        ],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2017 CHI Conference Extended Abstracts on Human Factors in Computing Systems',
        paper: 'kim2017sensors.pdf',
        slide: 'kim2017sensors.pdf',
    },
    kim2017s: {
        year: 2017,
        title:
            "It's More than Just Sharing Game Play Videos! Understanding User Motives in Mobile Game Social Media",
        authors: [
            'Hayun Kim',
            'Lebogang Wame Molefi',
            'Auk Kim',
            'Woontack Woo',
            'Aviv Segev',
            'Uichin Lee',
        ],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2017 CHI Conference Extended Abstracts on Human Factors in Computing Systems',
        paper: 'kim2017s.pdf',
        slide: 'kim2017s.pdf',
    },
    park2017facilitating: {
        year: 2017,
        title: 'Facilitating Pervasive Community Policing on the Road with Mobile Roadwatch',
        authors: [
            'Sangkeun Park',
            'Emilia-Stefania Ilincai',
            'Jeungmin Oh',
            'Sujin Kwon',
            'Rabeb Mizouni',
            'Uichin Lee',
        ],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2017 CHI Conference on Human Factors in Computing Systems, May 2017',
        paper: 'park2017facilitating.pdf',
        slide: "park2017facilitating.pdf",
    },
    kim2017letspic: {
        year: 2017,
        title: 'LetsPic: Supporting In-situ Collaborative Photography over a Large Physical Space',
        authors: ['Auk Kim', 'Sungjoon Kang', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2017 CHI Conference on Human Factors in Computing Systems, May 2017',
        paper: 'kim2017letspic.pdf',
        slide: "kim2017letspic.pdf",
    },
    noh2016infrastructure: {
        year: 2016,
        title:
            'Infrastructure-free Collaborative Indoor Positioning Scheme for Time-critical Team Operations',
        authors: ['Youngtae Noh', 'Hirozumi Yamaguchi', 'Uichin Lee'],
        venue_alias: 'IEEE',
        venue:
            'IEEE Transactions on Systems, Man, and Cybernetics: Systems, Volume: 48, Issue: 3, March 2018',
        paper: 'noh2016infrastructure.pdf',
    },
    lee2016internet: {
        year: 2016,
        title: 'Internet of Vehicles: From Intelligent Grid to Autonomous Cars and Vehicular Fogs',
        authors: ['Eun-Kyu Lee', 'Mario Gerla', 'Giovanni Pau', 'Uichin Lee', 'Jae-Han Lim'],
        venue_alias: 'IJDSN',
        venue: 'International Journal of Distributed Sensor Networks, Vol. 12(9), September 2016',
        paper: 'lee2016internet.pdf',
    },
    chae2016cannibalism: {
        year: 2016,
        title: 'Cannibalism in Medical Topic Networks',
        authors: ['Suhyun Chae', 'Aviv Segev', 'Uichin Lee'],
        venue_alias: 'Knowledge-Based Systems',
        venue: 'Knowledge-Based Systems, Volume 108, Sept. 15, 2016, Pages 168-178',
        paper: 'chae2016cannibalism.pdf',
    },
    choi2016exploring: {
        year: 2016,
        title:
            'Exploring User Experiences of Active Workstations: A Case Study of Under Desk Elliptical Trainers',
        authors: ['Woohyeok Choi', 'Aejin Song', 'Darren Edge', 'Masaaki Fukumoto', 'Uichin Lee'],
        venue_alias: 'UbiComp',
        venue:
            'Proceedings of the 2016 ACM International Joint Conference on Pervasive and Ubiquitous Computing, September 2016',
        paper: 'choi2016exploring.pdf',
        slide: "choi2016exploring.pdf",
    },
    choi2016designing: {
        year: 2016,
        title: 'Designing Interactive Multiswimmer Exergames: A Case Study',
        authors: [
            'Woohyeok Choi',
            'Jeungmin Oh',
            'Taiwoo Park',
            'Seongjun Kang',
            'Miri Moon',
            'Uichin Lee',
            'Inseok Hwang',
            'Darren Edge',
            'Junehwa Song',
        ],
        venue_alias: 'TOSN',
        venue: 'ACM Transaction on Sensor Networks, Volume 12, Issue 3, Article 17, July 2016',
        paper: 'choi2016designing.pdf',
    },
    shin2016motion: {
        year: 2016,
        title: 'Motion-MiX DHT for Wireless Mobile Networks',
        authors: ['Seungjae Shin', 'Uichin Lee', 'Falko Dressler', 'Hyunsoo Yoon'],
        venue_alias: 'IEEE',
        venue: 'IEEE Transaction on Mobile Computing, Volume 15, Issue 12, Dec. 1 2016',
        paper: 'shin2016motion.pdf',
    },
    choi2016lockdoll: {
        year: 2016,
        title: 'LockDoll: Providing Ambient Feedback of Smartphone Usage within Social Interaction',
        authors: ['Seungwoo Choi', 'Hayeon Jeong', 'Minsam Ko', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2016 CHI Conference Extended Abstracts on Human Factors in Computing Systems, May 2016',
        paper: 'choi2016lockdoll.pdf',
        slide: 'choi2016lockdoll.pdf',
    },
    kim2016ecomeal: {
        year: 2016,
        title: 'EcoMeal: A Smart Tray for Promoting Healthy Dietary Habits',
        authors: ['Jaejeung Kim', 'Joonyoung Park', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2016 CHI Conference Extended Abstracts on Human Factors in Computing Systems, May 2016',
        paper: 'kim2016ecomeal.pdf',
        slide: 'kim2016ecomeal.pdf',
    },
    jung2016social: {
        year: 2016,
        title:
            'Social or Financial Goals? Comparative Analysis of User Behaviors in Couchsurfing and Airbnb',
        authors: [
            'Jiwon Jung',
            'Susik Yoon',
            'SeungHyun Kim',
            'Sangkeun Park',
            'Kun-Pyo Lee',
            'Uichin Lee',
        ],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2016 CHI conference extended abstracts on human factors in computing systems, May 2016',
        paper: 'jung2016social.pdf',
        slide: 'jung2016social.pdf',
    },
    oh2016report: {
        year: 2016,
        title: 'How to Report App Feedback? Analyzing Feedback Reporting Behavior',
        authors: ['Jeungmin Oh', 'SangJeong Lee', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2016 CHI Conference Extended Abstracts on Human Factors in Computing Systems, May 2016',
        paper: 'oh2016report.pdf',
        slide: 'oh2016report.pdf',
    },
    kim2016slideqa: {
        year: 2016,
        title: 'SlideQA: Supporting Effective Q&A in an Offline Academic Presentation',
        authors: ['Juyoun Kim', 'Yoochan Kim', 'Sangkeun Park', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 2016 CHI Conference Extended Abstracts on Human Factors in Computing Systems, May 2016',
        paper: 'kim2016slideqa.pdf',
        slide: 'kim2016slideqa.pdf',
    },
    choi2016swimtrain: {
        year: 2016,
        title: 'SwimTrain: Exploring Exergame Design for Group Fitness Swimming',
        authors: ['Woohyeok Choi', 'Jeungmin Oh', 'Darren Edge', 'Joohyun Kim', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2016 CHI Conference on Human Factors in Computing Systems, May 2016',
        paper: 'choi2016swimtrain.pdf',
        slide: 'choi2016swimtrain.pdf',
        video: "https://youtu.be/AmVvpktm0QY",
    },
    park2016motives: {
        year: 2016,
        title: 'Motives and Concerns of Dashcam Video Sharing',
        authors: ['Sangkeun Park', 'Joohyun Kim', 'Rabeb Mizouni', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2016 CHI Conference on Human Factors in Computing Systems, May 2016',
        paper: 'park2016motives.pdf',
        slide: 'park2016motives.pdf',
    },
    ko2016lock: {
        year: 2016,
        title:
            "Lock n'LoL: Group-based Limiting Assistance App to Mitigate Smartphone Distractions in Group Activities",
        authors: ['Minsam Ko', 'Seungwoo Choi', 'Koji Yatani', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 2016 CHI Conference on Human Factors in Computing Systems, May 2016 (Best Paper Award)',
        paper: 'ko2016lock.pdf',
    },
    shin2016analysis: {
        year: 2016,
        title: 'Analysis of Cell Sojourn Time in Heterogeneous Networks with Small Cells',
        authors: ['Seungjae Shin', 'Uichin Lee', 'Falko Dressler', 'Hyunsoo Yoon'],
        venue_alias: 'IEEE',
        venue: 'IEEE Communications Letters, Volume 20, Issue 4, pp 788-791, April 2016',
        paper: 'shin2016analysis.pdf',
    },
    ko2016makes: {
        year: 2016,
        title:
            'What Makes Sports Fans Interactive? Identifying Factors Affecting Chat Interactions in Online Sports Viewing',
        authors: ['Minsam Ko', 'Jaeryong Yeo', 'Juyeong Lee', 'Uichin Lee', 'Young Jae Jang'],
        venue_alias: 'PLOS ONE',
        venue: 'PLOS ONE, February 5, 2016',
        paper: 'ko2016makes.pdf',
        code: 'https://zenodo.org/record/34939#.Vnt0dRWLTcu',
    },
    ko2016understanding: {
        year: 2016,
        title:
            'Understanding Mass Interactions in Online Sports Viewing: Chatting Motives and Usage Patterns',
        authors: ['Minsam Ko', 'Seungwoo Choi', 'Joonwon Lee', 'Uichin Lee', 'Aviv Segev'],
        venue_alias: 'TOCHI',
        venue: 'ACM Transactions on Computer-Human Interaction, Volume 23 Issue 1, February 2016',
        paper: 'ko2016understanding.pdf',
        code: 'https://zenodo.org/record/34939#.Vnt0dRWLTcu',
        slide: 'ko2016understanding.pdf',
    },
    ko2015familync: {
        year: 2015,
        title: "FamiLync: Facilitating Participatory Parental Mediation of Adolescents' Smartphone Use",
        authors: ['Minsam Ko', 'Seungwoo Choi', 'Subin Yang', 'Joonwon Lee', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue:
            'Proceedings of the 2015 ACM International Joint Conference on Pervasive and Ubiquitous Computing, Spetember 2015',
        paper: 'ko2015familync.pdf',
        slide: 'ko2015familync.pdf',
    },
    choi2015designing: {
        year: 2015,
        title: 'Designing Group Fitness Swimming Exergames: A Case Study',
        authors: ['Woohyeok Choi', 'Joohyun Kim', 'Jeungmin Oh', 'Darren Edge', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue:
            'Adjunct Proceedings of the 2015 ACM International Joint Conference on Pervasive and Ubiquitous Computing and Proceedings of the 2015 ACM International Symposium on Wearable Computers, September 2015',
        paper: 'choi2015designing.pdf',
        slide: 'choi2015designing.pdf',
    },
    jeong2015eng: {
        year: 2015,
        title: 'I-Eng: An Interactive Toy for Second Language Learning',
        authors: ['Hayeon Jeong', 'Daniel Pieter Saakes', 'Uichin Lee'],
        venue_alias: 'Ubicomp',
        venue:
            'Adjunct Proceedings of the 2015 ACM International Joint Conference on Pervasive and Ubiquitous Computing and Proceedings of the 2015 ACM International Symposium on Wearable Computers, September 2015',
        paper: 'jeong2015eng.pdf',
        slide: 'jeong2015eng.pdf',
    },
    kim2015crowdcolor: {
        year: 2015,
        title: 'CrowdColor: Crowdsourcing Color Perceptions Using Mobile Devices',
        authors: [
            'Jaejeung Kim',
            'Sergey Leksikov',
            'Punyotai Thamjamrassri',
            'Uichin Lee',
            'Hyeon-Jeong Suk',
        ],
        venue_alias: 'MobileHCI',
        venue:
            'Proceedings of the 17th International Conference on Human-Computer Interaction with Mobile Devices and Services, August 2015',
        paper: 'kim2015crowdcolor.pdf',
        slide: 'kim2015crowdcolor.pdf',
    },
    noh2015hydrocast: {
        year: 2015,
        title: 'HydroCast: Pressure Routing for Underwater Sensor Networks',
        authors: [
            'Youngtae Noh',
            'Uichin Lee',
            'Saewoom Lee',
            'Paul Wang',
            'Luiz F. M. Vieira',
            'Jun-Hong Cui',
            'Mario Gerla',
            'Kiseon Kim',
        ],
        venue_alias: 'IEEE',
        venue: 'IEEE Transaction on Vehicular Technology, Volume 65, Issue 1, January 2016',
        paper: 'noh2015hydrocast.pdf',
    },
    oh2015scanshot: {
        year: 2015,
        title: 'ScanShot: Detecting Document Capture Moments and Correcting Device Orientation',
        authors: ['Jeungmin Oh', 'Woohyeok Choi', 'Joohyun Kim', 'Uichin Lee'],
        venue_alias: 'CHI',
        venue:
            'Proceedings of the 33rd Annual ACM Conference on Human Factors in Computing Systems, April 2015',
        paper: 'oh2015scanshot.pdf',
        slide: 'oh2015scanshot.pdf',
    },
    ko2015lock: {
        year: 2015,
        title: "Lock n' LoL: Mitigating Smartphone Disturbance in Co-located Social Interactions",
        authors: [
            'Minsam Ko',
            'Chayanin Wong',
            'Sunmin Son',
            'Euigon Jung',
            'Uichin Lee',
            'Seungwoo Choi',
            'Sungho Jo',
            'Min H. Kim',
        ],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 33rd Annual ACM Conference Extended Abstracts on Human Factors in Computing Systems, April 2015',
        paper: 'ko2015lock.pdf',
        slide: 'ko2015lock.pdf',
    },
    song2015every: {
        year: 2015,
        title:
            'Every Little Helps: Understanding Donor Behavior in a Crowdfunding Platform for Non-profits',
        authors: ['Aejin Song', 'Hong-in Lee', 'Minsam Ko', 'Uichin Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 33rd Annual ACM Conference Extended Abstracts on Human Factors in Computing Systems, April 2015',
        paper: 'song2015every.pdf',
        slide: 'song2015every.pdf',
    },
    lee2015socialkeyboard: {
        year: 2015,
        title: 'SocialKeyboard: Proofreading Everyday Writings in Mobile Phones',
        authors: ['Jin-woo Lee', 'Joohyun Kim', 'Uichin Lee', 'Jae-Gil Lee'],
        venue_alias: 'CHI EA',
        venue:
            'Proceedings of the 33rd Annual ACM Conference Extended Abstracts on Human Factors in Computing Systems, April 2015',
        paper: 'lee2015socialkeyboard.pdf',
    },
    ko2015nugu: {
        year: 2015,
        title:
            'NUGU: A Group-based Intervention App for Improving Self-Regulation of Limiting Smartphone Use',
        authors: [
            'Minsam Ko',
            'Subin Yang',
            'Joonwon Lee',
            'Christian Heizmann',
            'Jinyoung Jeong',
            'Uichin Lee',
            'Daehee Shin',
            'Koji Yatani',
            'Junehwa Song',
            'Kyong-Mee Chung',
        ],
        venue_alias: 'CSCW',
        venue:
            'Proceedings of the 18th ACM Conference on Computer Supported Cooperative Work & Social Computing, February 2015',
        paper: 'ko2015nugu.pdf',
        slide: 'ko2015nugu.pdf',
    },
    oh2015exploring: {
        year: 2015,
        title: 'Exploring UX Issues in Quantified Self Technologies',
        authors: ['Jeungmin Oh', 'Uichin Lee'],
        venue_alias: 'ICMU',
        venue:
            '2015 Eighth International Conference on Mobile Computing and Ubiquitous Networking',
        paper: 'oh2015exploring.pdf',
        slide: 'oh2015exploring.pdf',
    },

    // bookmark

    이의진2015iot: {
        year: 2015,
        title: 'IoT 응용 설득 컴퓨팅 서비스 개요 및 연구 동향',
        authors: ['이의진'],
        venue_alias: 'JCCI',
        venue: 'JCCI 2015, 4/15-17, 2015',
        paper: '', // no pdf
    },
    양수빈2014청소년: {
        year: 2015,
        title: '청소년 자녀의 스마트폰 사용에 관한 부모중재 서비스 디자인 탐구',
        authors: ['양수빈', '고민삼', '이의진'],
        venue_alias: 'HCIK',
        venue: 'HCIK 2015, 12/10-12, 2014',
        paper: '양수빈2014청소년.pdf',
    },
    오태화2014활동이론에: {
        year: 2015,
        title:
            '활동이론에 근거한 대학원생의 교내 융합연구관련 Q&A 행동 분석 및 시스템 디자인 가이드라인',
        authors: ['오태화', '박상근', '이의진', '윤완철'],
        venue_alias: 'HCIK',
        venue: 'HCIK 2015, 12/10-12, 2014',
        paper: '오태화2014활동이론에.pdf',
    },
    최우혁2014수영: {
        year: 2015,
        title: '수영 환경을 고려한 다중 사용자 운동게임 디자인',
        authors: ['최우혁', '이의진'],
        venue_alias: 'HCIK',
        venue: 'HCIK 2015, 12/10-12, 2014',
        paper: '최우혁2014수영.pdf',
    },
    lee2015secure: {
        year: 2014,
        title: 'Secure Personal Content Networking over Untrusted Devices',
        authors: ['Uichin Lee', 'Josh Joy', 'Youngtae Noh'],
        venue: 'Wireless Personal Communications, Volume 80, Issue 4, pp 1449-1473, February 2015',
        venue_alias: 'Wireless Personal Communications',
        published: true,
        paper: 'lee2015secure.pdf',
        slide: 'lee2015secure.pdf',
    },
    bao2014methods: {
        year: 2014,
        title: 'Methods of locating data spots and networks and user equipment for using the same',
        authors: ['Xuan Bao', 'Uichin Lee', 'Ivica Rimac'],
        venue: 'US Patent US8914041 B2, Dec 16, 2014',
        venue_alias: 'Google Patents',
        published: true,
        paper: 'bao2014methods.pdf',
    },
    choi2014mobydick: {
        venue: 'Proceedings of the 12th ACM Conference on Embedded Network Sensor Systems',
        venue_alias: 'SenSys',
        published: true,
        paper: 'choi2014mobydick.pdf',
        year: 2014,
        title: 'MobyDick: An Interactive Multi-swimmer Exergame',
        authors: [
            'Woohyeok Choi',
            'Jeungmin Oh',
            'Taiwoo Park',
            'Sungjun Kang',
            'Miri Moon',
            'Uichin Lee',
            'Inseok Hwang',
            'Junehwa Song',
        ],
        slide: 'choi2014mobydick.pdf',
        video: "https://youtu.be/5JwIsf6SQ0Y",
        // venue:
        //   "In Proc. of the 12th ACM Conference on Embedded Networked Sensor Systems (SenSys'14), Memphis, TN, November 3-6, 2014",
    },
    park2014understanding: {
        // venue: 'Proceedings of the 16th international conference on Human-computer interaction with mobile devices \& services',
        venue_alias: 'MobileHCI',
        published: true,
        paper: 'park2014understanding.pdf',
        slide: 'park2014understanding.pdf',
        year: 2014,
        title: 'Understanding Localness of Knowledge Sharing: A Study of Naver KiN Here',
        authors: ['Sangkeun Park', 'Yongsung Kim', 'Uichin Lee', 'Mark Ackerman'],
        venue:
            "In Proc. of the 16th International Conference on Human-Computer Interaction with Mobile Devices and Services (MobileHCI'14), Toronto, Canada, Sept. 23-26, 2014",
    },
    cho2015placewalker: {
        year: 2014,
        title:
            'PlaceWalker: An Energy-efficient Place Logging Method that Considers Kinematics of Normal Human Walking',
        authors: ['Dae-Ki Cho', 'Uichin Lee', 'Youngtae Noh', 'Taiwoo Park', 'Junehwa Song'],
        venue: 'Pervasive and Mobile Computing',
        venue_alias: 'PMC',
        published: true,
        paper: 'cho2015placewalker.pdf',
    },
    park2014human: {
        year: 2014,
        title: 'Human Factors of Speed-based Exergame Controllers',
        authors: [
            'Taiwoo Park',
            'Uichin Lee',
            'I. Scott MacKenzie',
            'Miri Moon',
            'Inseok Hwang',
            'Junehwa Song',
        ],
        venue: 'Proceedings of the SIGCHI Conference on Human Factors in Computing Systems',
        venue_alias: 'CHI',
        published: true,
        paper: 'park2014human.pdf',
    },
    lee2014hooked: {
        year: 2014,
        title:
            'Hooked on Smartphones: An Exploratory Study on Smartphone Overuse among College Students',
        authors: [
            'Uichin Lee',
            'Joonwon Lee',
            'Minsam Ko',
            'Changhun Lee',
            'Yuhwan Kim',
            'Subin Yang',
            'Koji Yatani',
            'Gahgene Gweon',
            'Kyong-Mee Chung Junehwa Song',
        ],
        venue: 'Proceedings of the SIGCHI conference on human factors in computing systems',
        venue_alias: 'CHI',
        published: true,
        paper: 'lee2014hooked.pdf',
        slide: 'lee2014hooked.pdf',
    },

    /////////////////
    // Emily start //
    /////////////////

    baumer2014refusing: {
        year: 2014,
        title:
            'Supporting Temporary Non-Use of Smartphones Refusing, Limiting, Departing: Why We Should Study Technology Non-use',
        authors: ['Uichin Lee', 'Subin Yang', 'Minsam Ko', 'Joonwon Lee'],
        venue: 'CHI14 Extended Abstracts on Human Factors in Computing Systems',
        venue_alias: 'CHI',
        published: true,
        paper: 'baumer2014refusing.pdf',
    },
    ko2014motives: {
        year: 2014,
        title: 'Motives for Mass Interactions in Online Sports Viewing',
        authors: [
            'Minsam Ko',
            'Seung-woo Choi',
            'Joonwon Lee',
            'Subin Yang',
            'Uichin Lee',
            'Aviv Segev',
            'Junehwa Song',
        ],
        venue: 'Proceedings of the 23rd International Conference on World Wide Web',
        venue_alias: 'WWW',
        published: true,
        paper: 'ko2014motives.pdf',
    },
    gerla2014internet: {
        year: 2014,
        title: 'Internet of Vehicles: From Intelligent Grid to Autonomous Cars and Vehicular Clouds',
        authors: ['Mario Gerla', 'Eun-Kyu Lee', 'Giovanni Pau', 'Uichin Lee'],
        venue: '2014 IEEE world forum on internet of things (WF-IoT)',
        venue_alias: 'WF-IoT',
        published: true,
        paper: 'gerla2014internet.pdf',
        slide: 'gerla2014internet.pdf',
    },
    noh2014dots: {
        year: 2014,
        title:
            'DOTS: A Propagation Delay-aware Opportunistic MAC Protocol for Mobile Underwater Networks',
        authors: [
            'Youngtae Noh',
            'Uichin Lee',
            'Seongwon Han',
            'Paul Wang',
            'Dustin Torres',
            'Jinwhan Kim',
            'Mario Gerla',
        ],
        venue: 'IEEE Transactions on Mobile Computing',
        venue_alias: 'IEEE CASS',
        published: true,
        paper: 'noh2014dots.pdf',
    },
    고민삼2014스마트폰: {
        year: 2014,
        title: '스마트폰 과도사용 중재 모바일 앱 분석: 중재 방법 관점',
        authors: ['고민삼', '이준원', '양수빈', '이의진'],
        venue: 'HCI Korea, Feb. 2014',
        // venue: '한국 HCI 학회 학술대회',
        venue_alias: 'HCI Korea',
        published: true,
        paper: '고민삼2014스마트폰.pdf',
    },
    강성준2014다중: {
        year: 2014,
        title: '다중 스마트 단말 협업 기반 모바일 어플리케이션 및 UX 디자인 고찰',
        authors: ['강성준', '신대희', '명미라', '이의진', '박준성'],
        venue: 'HCI Korea, Feb. 2014',
        // venue: '한국 HCI 학회 학술대회',
        venue_alias: 'HCI Korea',
        published: true,
        paper: '강성준2014다중.pdf',
    },
    lee2013understanding: {
        year: 2013,
        title:
            'Understanding Processing Overheads of Network Coding Based Content Distribution in VANETs',
        authors: ['Uichin Lee', 'Seung-Hoon Lee', 'Kang-Won Lee', 'Mario Gerla'],
        venue: 'IEEE Transactions on Parallel and Distributed Systems',
        venue_alias: 'IEEE TPDS',
        published: true,
        paper: 'lee2013understanding.pdf',
    },
    sung2013booming: {
        year: 2013,
        title:
            'Booming Up the Long Tails: Discovering Potentially Contributive Users in Community-based Question Answering Services',
        authors: ['Juyup Sung', 'Jae-Gil Lee', 'Uichin Lee'],
        venue: 'Proceedings of the International AAAI Conference on Web and Social Media',
        venue_alias: 'ICWSM',
        published: true,
        paper: 'sung2013booming.pdf',
    },
    lee2013sociophone: {
        year: 2013,
        title:
            'SocioPhone: Everyday Face-to-Face Interaction Monitoring Platform Using Multi-Phone Sensor Fusion',
        authors: [
            'Youngki Lee',
            'Chulhong Min',
            'Chanyou Hwang',
            'Jaeung Lee',
            'Inseok Hwang',
            'Younghyun Ju',
            'Chungkuk Yoo',
            'Miri Moon',
            'Uichin Lee',
            'Junehwa Song',
        ],
        venue:
            'Proceeding of the 11th annual international conference on Mobile systems, applications, and services',
        venue_alias: 'MobiSys',
        published: true,
        paper: 'lee2013sociophone.pdf',
    },
    lee2013analyzing: {
        year: 2013,
        title: 'Analyzing Crowd Workers in Mobile Pay-for-Answer Q&A',
        authors: ['Uichin Lee', 'Jihyoung Kim', 'Eunhee Yi', 'Juyup Sung', 'Mario Gerla'],
        venue: 'Proceedings of the SIGCHI conference on human factors in computing systems',
        venue_alias: 'CHI',
        published: true,
        paper: 'lee2013analyzing.pdf',
        slide: 'lee2013analyzing.pdf',
    },
    oh2013facilitating: {
        year: 2013,
        title: 'Facilitating Developer-User Interactions with Mobile App Review Digests',
        authors: ['Jeungmin Oh', 'Daehoon Kim', 'Uichin Lee', 'Jae-Gil Lee', 'Junehwa Song'],
        venue: 'CHI13 Extended Abstracts on Human Factors in Computing Systems',
        venue_alias: 'CHI',
        published: true,
        paper: 'oh2013facilitating.pdf',
    },
    bao2013dataspotting: {
        year: 2013,
        title:
            'DataSpotting: Exploiting Naturally Clustered Mobile Devices to Offload Cellular Traffic',
        authors: ['Xuan Bao', 'Yin Lin', 'Uichin Lee', 'Ivica Rimac', 'Romit Roy Choudhury'],
        venue: '2013 Proceedings IEEE INFOCOM',
        venue_alias: 'IEEE INFOCOM',
        published: true,
        paper: 'bao2013dataspotting.pdf',
    },
    han2013m: {
        year: 2013,
        title: 'M-FAMA: A Multi-session MAC Protocol for Reliable Underwater Acoustic Streams',
        authors: ['Seongwon Han', 'Youngtae Noh', 'Uichin Lee', 'Mario Gerla'],
        venue: '2013 Proceedings IEEE INFOCOM',
        venue_alias: 'IEEE INFOCOM',
        published: true,
        paper: 'han2013m.pdf',
        slide: 'han2013m.pdf',
    },
    noh2013infrastructure: {
        venue: 'IEEE Transactions on Systems, Man, and Cybernetics: Systems',
        venue_alias: 'IEEE SMC',
        published: true,
        paper: 'noh2013infrastructure.pdf',
        year: 2013,
        title:
            'CLIPS: Infrastructure-free Collaborative Indoor Positioning Scheme for Time-critical Team Operations',
        authors: [
            'Youngtae Noh',
            'Hirozumi Yamaguchi',
            'Uichin Lee',
            'Prerna Vij',
            'Joshua Joy',
            'Mario Gerla',
        ],
        // venue: 'IEEE PerCom 2013, San Diego, CA, March 18-22, 2013',
    },
    park2013exersync: {
        venue: 'Proceedings of the 2013 conference on Computer supported cooperative work',
        venue_alias: 'CSCW',
        published: true,
        paper: 'park2013exersync.pdf',
        year: 2013,
        title: 'ExerSync: Facilitating Interpersonal Synchrony in Social Exergames',
        authors: [
            'Taiwoo Park',
            'Uichin Lee',
            'Bupjae Lee',
            'Haechan Lee',
            'Sanghun Son',
            'Seokyoung Song',
            'Junehwa Song',
        ],
    },
    lee2013mobile: {
        venue: 'Proceedings of CHI',
        venue_alias: 'CHI',
        published: false,
        paper: '', // no pdf
        year: 2013,
        title: 'Mobile Q&A: Beyond Text-only Q&A and Privacy Concerns',
        authors: ['Uichin Lee', 'Eunhee Yi', 'Minsam Ko'],
    },
    lee2013dungeons: {
        year: 2013,
        title: 'Dungeons & Swimmers: Designing an Interactive Exergame for Swimming,',
        authors: [
            'Haechan Lee',
            'Miri Moon',
            'Taiwoo Park',
            'Inseok Hwang',
            'Uichin Lee',
            'Junehwa Song',
        ],
        venue:
            'Proceedings of the 2013 ACM conference on Pervasive and Ubiquitous Computing adjunct publication',
        venue_alias: 'Ubicomp',
        published: true,
        paper: 'lee2013dungeons.pdf',
    },
    park2012exerlink: {
        venue:
            'Proceedings of the 10th international conference on Mobile systems, applications, and services',
        venue_alias: 'MobiSys',
        published: true,
        paper: 'park2012exerlink.pdf',
        year: 2012,
        title: 'ExerLink: enabling pervasive social exergames with heterogeneous exercise devices',
        authors: ['Park et al.'],
    },
    lee2012understanding: {
        year: 2012,
        title: 'Understanding Mobile Q&A Usage: An Exploratory Study',
        authors: ['Uichin Lee', 'Hyanghong Kang', 'Eunhee Yi', 'Mun Y. Yi', 'Jussi Kantola'],
        venue: 'Proceedings of the SIGCHI Conference on Human Factors in Computing Systems',
        venue_alias: 'CHI',
        published: true,
        paper: 'lee2012understanding.pdf',
        slide: 'lee2012understanding.pdf',
    },
    noh2013vapr: {
        authors: ['Youngtae Noh', 'Uichin Lee', 'Paul Wang', 'Brian Sung Chul Choi', 'Mario Gerla'],
        title: 'VAPR: Void Aware Pressure Routing for Underwater Sensor Networks',
        venue: 'IEEE Transactions on Mobile Computing',
        venue_alias: 'IEEE TMC',
        year: 2013,
        published: true,
        paper: 'noh2013vapr.pdf',
    },
    bellavista2012editorial: {
        authors: ['Paolo Bellavista', 'Mario Gerla', 'Hariharan Krishnan', 'Uichin Lee'],
        title: 'Editorial - Special Issue: Wide-Scale Vehicular Sensor Networks and Mobile Sensing',
        venue: 'Pervasive and Mobile Computing',
        venue_alias: '',
        year: 2012,
        published: true,
        paper: 'bellavista2012editorial.pdf',
    },
    lee2012emerging: {
        authors: ['Uichin Lee', 'Howon Lee', 'Bang Chul Jung', 'Junehwa Song'],
        title:
            'Emerging Ubiquitous Knowledge Services: From Mobile Sensing to Ubiquitous Crowdsourcing and Beyond',
        venue: 'Knowledge service engineering handbook',
        venue_alias: '',
        year: 2011,
        published: true,
        paper: 'lee2012emerging.pdf',
    },
    ahnn2011geoserv: {
        authors: ['Jong Hoon Ahn', 'Uichin Lee', 'Hyun Jin Moon'],
        title: 'GeoServ: A Distributed Urban Sensing Platform',
        venue: '2011 11th IEEE/ACM International Symposium on Cluster, Cloud and Grid Computing',
        venue_alias: 'IEEE CCGRID',
        year: 2011,
        published: true,
        paper: 'ahnn2011geoserv.pdf',
        slide: 'ahnn2011geoserv.pdf',
    },
    lim2013sewersnort: {
        authors: [
            'Jung Soo Lim',
            'Jihyoung Kim',
            'Jonathan Friedman',
            'Uichin Lee',
            'Luiz Vieira',
            'Diego Rosso',
            'Mario Gerla',
            'Mani B Srivastava',
        ],
        title: 'SewerSnort: A Drifting Sensor for In-situ Wastewater Collection System Gas Monitoring',
        venue: 'Ad hoc networks',
        venue_alias: '',
        year: 2011,
        published: true,
        paper: 'lim2013sewersnort.pdf',
        slide: 'lim2013sewersnort.pdf',
    },
    lee2011toward: {
        authors: ['Uichin Lee', 'Ivica Rimac', 'Daniel Kilper', 'Volker Hilt'],
        title: 'Toward Energy Efficient Content Dissemination',
        venue: 'IEEE Network',
        venue_alias: 'IEEE',
        year: 2011,
        published: true,
        paper: 'lee2011toward.pdf',
    },
    varvello2011design: {
        authors: ['Matteo Varvello', 'Ivica Rimac', 'Uichin Lee', 'Lloyd Greenwald', 'Volker Hilt'],
        title: 'On the Design of Content-Centric MANETs',
        venue:
            '2011 Eighth International Conference on Wireless On-Demand Network Systems and Services',
        venue_alias: 'WONS',
        year: 2011,
        published: true,
        paper: 'varvello2011design.pdf',
    },
    noh2010dots: {
        title:
            'DOTS: A Propagation Delay-aware Opportunistic MAC Protocol for Underwater Sensor Networks',
        authors: ['Youngtae Noh', 'Paul Wang', 'Uichin Lee', 'Dustin Torres', 'Mario Gerla'],
        venue: 'The 18th IEEE International Conference on Network Protocols',
        venue_alias: 'IEEE ICNP',
        year: 2010,
        published: true,
        paper: 'noh2010dots.pdf',
        slide: 'noh2010dots.pdf',
    },
    noh2012vapr: {
        authors: ['Youngtae Noh', 'Paul Wang', 'Uichin Lee', 'Mario Gerla'],
        title: 'VAPR: Void Aware Pressure Routing Protocol',
        venue: 'IEEE Transactions on Mobile Computing',
        venue_alias: 'IEEE',
        year: 2010,
        published: true,
        paper: 'noh2012vapr.pdf',
    },
    lee2010geo: {
        authors: ['Kevin C. Lee', 'Uichin Lee', 'Mario Gerla'],
        title: 'Geo-Opportunistic Routing for Vehicular Networks',
        venue: 'IEEE Communications Magazine',
        venue_alias: 'IEEE',
        year: 2010,
        published: true,
        paper: 'lee2010geo.pdf',
    },
    lee2010trace: {
        authors: ['Kevin C. Lee', 'Juan M. Navarro', 'Tin Y. Chong', 'Uichin Lee', 'Mario Gerla'],
        title: 'Trace-based Evaluation of Rate Adaptation Schemes in Vehicular Environments',
        venue: '2010 IEEE 71st Vehicular Technology Conference',
        venue_alias: 'IEEE VTC',
        year: 2010,
        published: true,
        paper: 'lee2010trace.pdf',
    },
    lee2010greening: {
        authors: ['Uichin Lee', 'Ivica Rimac', 'Volker Hilt'],
        title: 'Greening the Internet with Content Centric Networking',
        venue:
            'Proceedings of the 1st International Conference on Energy-efficient Computing and Networking',
        venue_alias: 'E-Energy',
        year: 2010,
        published: true,
        paper: 'lee2010greening.pdf',
        slide: 'lee2010greening.pdf',
    },
    cho2010autogait: {
        authors: ['Dae-Ki Cho', 'Min Mun', 'Uichin Lee', 'William J. Kaiser', 'Mario Gerla'],
        title: 'AutoGait: A Mobile Platform that Accurately Estimates the Distance Walked',
        venue:
            '2010 IEEE international conference on pervasive computing and communications (PerCom)',
        venue_alias: 'PerCom',
        year: 2010,
        published: true,
        paper: 'cho2010autogait.pdf',
        slide: 'cho2010autogait.pdf',
    },
    park2010vehicular: {
        authors: ['Joon-Sang Park', 'Uichin Lee', 'Mario Gerla'],
        title: 'Vehicular Communications: Emergency Video Streams and Network Coding',
        venue: 'Journal of Internet Services and Applications',
        venue_alias: 'JISA',
        year: 2010,
        published: true,
        paper: 'park2010vehicular.pdf',
    },
    lee2010pressure: {
        authors: [
            'Uichin Lee',
            'Paul Wang',
            'Youngtae Noh',
            'Luiz F. M. Vieira',
            'Mario Gerla',
            'Jun-Hong Cui',
        ],
        title: 'Pressure Routing for Underwater Sensor Networks',
        venue: '2010 Proceedings IEEE INFOCOM',
        venue_alias: 'IEEE INFOCOM',
        year: 2010,
        published: true,
        paper: 'lee2010pressure.pdf',
        slide: 'lee2010pressure.pdf',
    },
    lee2010surveyurban: {
        authors: ['Uichin Lee', 'Mario Gerla'],
        title: 'A Survey of Urban Vehicular Sensing Platforms',
        venue_alias: 'Computer Networks',
        venue: 'Computer Networks',
        year: 2010,
        published: true,
        paper: 'lee2010surveyurban.pdf',
    },
    lee2009fleanet: {
        authors: ['Uichin Lee', 'Jiyeon Lee', 'Joon-Sang Park', 'Mario Gerla'],
        title: 'FleaNet: A Virtual Market Place on Vehicular Networks',
        venue: 'IEEE Transactions on Vehicular Technology',
        venue_alias: 'IEEE',
        year: 2010,
        published: true,
        paper: 'lee2009fleanet.pdf',
        slide: 'lee2009fleanet.pdf',
    },
    lee2009p2p: {
        authors: ['Uichin Lee', 'Sewook Jung', 'Alexander Chang', 'Dae-Ki Cho', 'Mario Gerla'],
        title: 'P2P Content Distribution to Mobile Bluetooth Users',
        venue: 'IEEE Transactions on vehicular Technology',
        venue_alias: 'IEEE',
        year: 2010,
        published: true,
        paper: 'lee2009p2p.pdf',
    },
    lee2009scaling: {
        authors: ['Uichin Lee', 'Soon Young Oh', 'Kang-Won Lee', 'Mario Gerla'],
        title: 'Scaling Properties of Delay Tolerant Networks with Correlated Motion Patterns',
        venue: 'Proceedings of the 4th ACM Workshop on Challenged Networks',
        venue_alias: 'CHANTS',
        year: 2009,
        published: true,
        paper: 'lee2009scaling.pdf',
        slide: 'lee2009scaling.pdf',
    },
    lee2009relaycast: {
        authors: ['Uichin Lee', 'Soon Young Oh', 'Kang-Won Lee', 'Mario Gerla'],
        title: 'RelayCast: Scalable Multicast Routing in Delay Tolerant Coalition Networks',
        venue: 'Proc. Third Annual Conference of ITA',
        venue_alias: 'ITA',
        year: 2009,
        published: true,
        paper: 'lee2009relaycast.pdf',
    },
    kim2009sewersnort: {
        authors: [
            'Jihyoung Kim',
            'Jung Soo Lim',
            'Jonathan Friedman',
            'Uichin Lee',
            'Luiz Vieira',
            'Diego Rosso',
            'Mario Gerla',
            'Mani B Srivastava',
        ],
        title: 'SewerSnort: A Drifting Sensor for In-situ Sewer Gas Monitoring',
        venue:
            '2009 6th Annual IEEE Communications Society Conference on Sensor, Mesh and Ad Hoc Communications and Networks',
        venue_alias: 'SECON',
        year: 2009,
        published: true,
        paper: 'kim2009sewersnort.pdf',
    },
    lee2009go: {
        authors: ['Kevin C. Lee', 'Uichin Lee', 'Mario Gerla'],
        title: 'TO-GO: TOpology-assist Geo-Opportunistic Routing in Urban Vehicular Grids',
        venue:
            '2009 Sixth International Conference on Wireless On-Demand Network Systems and Services',
        venue_alias: 'WONS',
        year: 2009,
        published: true,
        paper: 'lee2009go.pdf',
        slide: 'lee2009go.pdf',
    },
    lee2010surveyrouting: {
        authors: ['Kevin C. Lee', 'Uichin Lee', 'Mario Gerla.'],
        title: 'Survey of Routing Protocols in Vehicular Ad Hoc Networks',
        venue: 'Advances in vehicular ad-hoc networks: Developments and challenges',
        venue_alias: 'VANET',
        year: 2009,
        published: true,
        paper: 'https://www.igi-global.com/chapter/advances-vehicular-hoc-networks/43169',  // need to pay for downloading pdf
    },
    lee2009bio: {
        authors: [
            'Uichin Lee',
            'Eugenio Magistretti',
            'Mario Gerla',
            'Paolo Bellavista',
            'Pietro Lio',
            'Kang-Won Lee',
        ],
        title: 'Bio-inspired Multi-Agent Data Harvesting in a Proactive Urban Monitoring Environment',
        venue: 'Ad Hoc Networks',
        venue_alias: '',
        year: 2009,
        published: true,
        paper: 'lee2009bio.pdf',
    },
    lee2008dissemination: {
        authors: [
            'Uichin Lee',
            'Eugenio Magistretti',
            'Mario Gerla',
            'Paolo Bellavista',
            'Antonio Corradi',
        ],
        title: 'Dissemination and Harvesting of Urban Data using Vehicular Sensor Platforms',
        venue: 'IEEE transactions on vehicular technology',
        venue_alias: 'IEEE',
        year: 2009,
        published: true,
        paper: 'lee2008dissemination.pdf',
    },
    lee2009emerging: {
        authors: ['Uichin Lee', 'Ryan Cheung', 'Mario Gerla'],
        title: 'Emerging Vehicular Applications',
        venue: 'Vehicular networks: From theory to practice',
        venue_alias: '',
        year: 2009,
        published: true,
        paper: 'lee2009emerging.pdf',
    },
    lee2008relaycast: {
        authors: ['Uichin Lee', 'Soon Young Oh', 'Kang-Won Lee', 'Mario Gerla'],
        title: 'RelayCast: Scalable Multicast Routing in Delay Tolerant Networks',
        venue: '2008 IEEE International Conference on Network Protocols',
        venue_alias: 'IEEE ICNP',
        year: 2008,
        published: true,
        paper: 'lee2008relaycast.pdf',
        slide: 'lee2008relaycast.pdf',
    },
    vieira2008phero: {
        authors: ['Luiz F. M. Vieira', 'Uichin Lee', 'Mario Gerla'],
        title: 'Phero-Trail: a Bio-inspired Location Service for Mobile Underwater Sensors',
        venue: 'Proceedings of the third ACM international workshop on Underwater Networks',
        venue_alias: 'WuWNet',
        year: 2008,
        published: true,
        paper: 'vieira2008phero.pdf',
        slide: 'vieira2008phero.pdf',
    },
    lee2007bio: {
        authors: [
            'Uichin Lee',
            'Eugenio Magistretti',
            'Mario Gerla',
            'Paolo Bellavista',
            'Pietro Lio',
            'Kang-Won Lee',
        ],
        title: 'Bio-inspired Multi-Agent Collaboration for Urban Monitoring Applications',
        venue: 'Workshop on Bio-Inspired Design of Networks',
        venue_alias: 'BIOWIRE',
        year: 2008,
        published: true,
        paper: 'lee2007bio.pdf',
    },
    lee2008content: {
        authors: ['Seung-Hoon Lee', 'Uichin Lee', 'Kang-Won Lee', 'Mario Gerla'],
        title:
            'Content Distribution in VANETs using Network Coding: The Effect of Disk I/O and Processing O/H',
        venue:
            '2008 5th Annual IEEE Communications Society Conference on Sensor, Mesh and Ad Hoc Communications and Networks',
        venue_alias: 'IEEE SECON',
        year: 2008,
        published: true,
        paper: 'lee2008content.pdf',
        slide: 'lee2008content.pdf',
    },
    lee2008efficient: {
        authors: [
            'Uichin Lee',
            'Joon-Sang Park',
            'Seung-Hoon Lee',
            'Won W. Ro',
            'Giovanni Pau',
            'Mario Gerla',
        ],
        title: 'Efficient Peer-to-peer File Sharing using Network Coding in MANET',
        venue: 'Journal of Communications and Networks',
        venue_alias: 'JCN',
        year: 2008,
        published: true,
        paper: 'lee2008efficient.pdf',
    },
    park2008delay: {
        authors: [
            'Joon-Sang Park',
            'Uichin Lee',
            'Soon Y. Oh',
            'Mario Gerla',
            'Desmond S. Lun',
            'Won W. Ro',
            'Joonseok Park',
        ],
        title:
            'Delay Analysis of Car-to-Car Reliable Data Delivery Strategies based on Data Muling with Network Coding',
        venue: 'IEICE transactions on information and systems',
        venue_alias: 'IEICE',
        year: 2008,
        published: true,
        paper: 'park2008delay.pdf',
    },
    gerla2008tapping: {
        authors: ['Mario Gerla', 'Uichin Lee'],
        title: 'Tapping Vehicle Sensors for Homeland Security',
        venue: '',
        venue_alias: '',
        year: 2008,
        published: true,
        paper: 'https://www.amazon.com/Homeland-Security-Technology-Challenges-Encrypting/dp/1596932899',   // have to pay for downloading pdf
    },
    lee2007enhanced: {
        authors: ['Kevin C. Lee', 'Jerome Haerri', 'Uichin Lee', 'Mario Gerla'],
        title:
            'Enhanced Perimeter Routing for Geographic Forwarding Protocols in Urban Vehicular Scenarios',
        venue: '2007 ieee globecom workshops',
        venue_alias: 'IEEE',
        year: 2007,
        published: true,
        paper: 'lee2007enhanced.pdf',
    },
    bellavista2007standard: {
        authors: ['P. Bellavista', 'E. Magistretti', 'U. Lee', 'and M. Gerla.'],
        title:
            'Standard Integration of Sensing and Opportunistic Diffusion for Urban Monitoring in Vehicular Sensor Networks: the MobEyes Architecture',
        venue: '2007 IEEE International Symposium on Industrial Electronics',
        venue_alias: 'IEEE ISIE',
        year: 2007,
        published: true,
        paper: 'bellavista2007standard.pdf',
    },
    magistretti2007mobile: {
        authors: [
            'Eugenio Magistretti',
            'Jiejun Kong',
            'Uichin Lee',
            'Mario Gerla',
            'Paolo Bellavista',
            'Antonio Corradi',
        ],
        title:
            'A Mobile Delay-tolerant Approach to Long-term Energy-efficient Underwater Sensor Networking',
        venue: '2007 IEEE wireless communications and networking conference',
        venue_alias: 'IEEE WCNC',
        year: 2007,
        published: true,
        paper: 'magistretti2007mobile.pdf',
    },
    jung2007bluetorrent: {
        authors: ['Sewook Jung', 'Uichin Lee', 'Alexander Chang', 'Dae-Ki Cho', 'Mario Gerla'],
        title: 'BlueTorrent: Cooperative Content Sharing for Bluetooth Users',
        venue: 'Pervasive and Mobile Computing',
        venue_alias: 'PMC',
        year: 2007,
        published: true,
        paper: 'jung2007bluetorrent.pdf',
        slide: 'jung2007bluetorrent.pdf',
    },
    lee2007time: {
        authors: ['Uichin Lee', 'Jiejun Kong', 'Eugenio Magistretti', 'Joon-Sang Park', 'Mario Gerla'],
        title:
            'Time-critical Underwater Sensor Diffusion with No Proactive Exchanges and Negligible Reactive Floods',
        venue: 'Ad Hoc Networks',
        venue_alias: '',
        year: 2007,
        published: true,
        paper: 'lee2007time.pdf',
    },
    lee2006mobeyes: {
        authors: [
            'Uichin Lee',
            'Eugenio Magistretti',
            'Biao Zhou',
            'Mario Gerla',
            'Paolo Bellavista',
            'Antonio Corradi',
        ],
        title: 'MobEyes: Smart Mobs for Urban Monitoring with a Vehicular Sensor Network',
        venue: 'IEEE Wireless Communications',
        venue_alias: 'IEEE',
        year: 2006,
        published: true,
        paper: 'lee2006mobeyes.pdf',
        slide: 'lee2006mobeyes.pdf',
    },
    lee2006code: {
        authors: ['Uichin Lee', 'Joon-Sang Park', 'Joseph Yeh', 'Giovanni Pau', 'Mario Gerla'],
        title: 'CodeTorrent: Content Distribution using Network Coding in VANETs',
        venue:
            'Proceedings of the 1st international workshop on Decentralized resource sharing in mobile computing and networking',
        venue_alias: 'MobiCom',
        year: 2006,
        published: true,
        paper: 'lee2006code.pdf',
        slide: 'lee2006code.pdf',
    },
    gerla2006vehicular: {
        authors: [
            'Mario Gerla',
            'Biao Zhou',
            'Yeng-Zhong Lee',
            'Fabio Soldo',
            'Uichin Lee',
            'Gustavo Marfia',
        ],
        title: 'Vehicular Grid Communications: The Role of the Internet Infrastructure',
        venue: 'Proceedings of the 2nd annual international workshop on Wireless internet',
        venue_alias: 'WICON',
        year: 2006,
        published: true,
        paper: 'gerla2006vehicular.pdf',
        slide: 'gerla2006vehicular.pdf',
    },
    lee2006efficient: {
        authors: [
            'Uichin Lee',
            'Eugenio Magistretti',
            'Biao Zhou',
            'Mario Gerla',
            'Paolo Bellavista',
            'Antonio Corradi',
        ],
        title: 'Efficient Data Harvesting in Mobile Sensor Platforms',
        venue:
            'Fourth Annual IEEE International Conference on Pervasive Computing and Communications Workshops',
        venue_alias: 'PERCOMW',
        year: 2006,
        published: true,
        paper: 'lee2006efficient.pdf',
    },
    lee2006understanding: {
        authors: ['Uichin Lee', 'Min Choi', 'Junghoo Cho', 'Medy. Y. Sanadidi', 'Mario Gerla'],
        title: 'Understanding Pollution Dynamics in P2P File Sharing',
        venue: 'IPTPS',
        venue_alias: 'IPTPS',
        year: 2006,
        published: true,
        paper: 'lee2006understanding.pdf',
        slide: 'lee2006understanding.pdf',
    },
    lee2005flowbased: {
        authors: ['Uichin Lee', 'Joon-Sang Park', 'M. Y. Sanadidi', 'M. Gerla'],
        title: 'Flow Based Dynamic Load Balancing for Passive Network Monitoring',
        venue: 'Communications and Computer Networks',
        venue_alias: '',
        year: 2005,
        published: false,
        paper: 'lee2005flowbased.pdf',
    },
    lee2005automatic: {
        authors: ['Uichin Lee', 'Zhenyu Liu', 'Junghoo Cho'],
        title: 'Automatic Identification of User Goals in Web Search',
        venue: 'Proceedings of the 14th international conference on World Wide Web',
        venue_alias: 'WWW',
        year: 2005,
        published: true,
        paper: 'lee2005automatic.pdf',
    },
    chen2009codecast: {
        authors: ['Chien-Chia Chen', 'Chieh-Ning Lien', 'Uichin Lee', 'Soon Y. Oh', 'Mario Gerla'],
        title: 'CodeCast: Network Coding Based Multicast in MANETs',
        venue:
            'Demos of the 10th international workshop on mobile computing systems and applications',
        venue_alias: 'HotMobile',
        year: 2009,
        published: true,
        paper: 'chen2009codecast.pdf',
    },
    cao2008proof: {
        authors: ['Zhen Cao', 'Jiejun Kong', 'Uichin Lee', 'Mario Gerla', 'Zhong Chen'],
        title:
            'Proof-of-Relevance: Filtering False Data via Authentic Consensus in Vehicle Ad-hoc Networks',
        venue: 'IEEE INFOCOM Workshops 2008',
        venue_alias: 'INFOCOM WKSHPS',
        year: 2008,
        published: true,
        paper: 'cao2008proof.pdf',
    },
    lee2008understanding: {
        authors: ['Uichin Lee', 'Kang-Won Lee', 'Soon-Young Oh', 'Mario Gerla'],
        title:
            'Understanding the Capacity and Delay Scaling Laws of Delay Tolerant Networks: A Unified Approach',
        venue: 'UCLA Technical Report: TR-070020',
        venue_alias: '',
        year: 2007,
        published: true,
        paper: 'lee2008understanding.pdf',
    },
    lee2007first: {
        authors: ['Kevin C. Lee', 'Seung-Hoon Lee', 'Ryan Cheung', 'Uichin Lee', 'Mario Gerla'],
        title: 'First Experience with CarTorrent in a Real Vehicular Ad Hoc Network Testbed',
        venue: '2007 Mobile Networking for Vehicular Environments',
        venue_alias: '',
        year: 2007,
        published: true,
        paper: 'lee2007first.pdf',
        slide: 'lee2007first.pdf',
    },
    vieira2006analysis: {
        authors: ['Luiz Filipe M. Vieira', 'Jiejun Kong', 'Uichin Lee', 'Mario Gerla'],
        title: 'Analysis of aloha protocols for underwater acoustic sensor networks',
        venue: 'Extended abstract from WUWNet',
        venue_alias: '',
        year: 2006,
        published: true,
        paper: 'vieira2006analysis.pdf',
    },
    park2006emergency: {
        authors: ['Joon-Sang Park', 'Uichin Lee', 'Soon Young Oh', 'Mario Gerla', 'Desmond Lun'],
        title: 'Emergency Related Video Streaming in VANETs using Network Coding',
        venue: 'Proceedings of the 3rd international workshop on Vehicular ad hoc networks',
        venue_alias: 'VANET',
        year: 2006,
        published: true,
        paper: 'park2006emergency.pdf',
    },
}


export type { Publication }
export default publications